/**
 ...
 SYSCFG_MemoryRemapConfig()
       ...
       (#) Swapping the internal flash Bank1 and Bank2 this features is only
 visible for STM32F42xxx/43xxx devices Devices.
...
#define UFB_MODE_BB                                                          \
	(PERIPH_BB_BASE + (MEMRMP_OFFSET * 32) + (UFB_MODE_BitNumber * 4))
...
#define MCHDLYCR_BSCKSEL_BB                                                  \
	(uint32_t)(PERIPH_BB_BASE + (MCHDLYCR_OFFSET * 32)                       \
	           + (BSCKSEL_BIT_NUMBER * 4))
...
/**
 * @brief  Deinitializes the Alternate Functions (remap and EXTI
 ...
 * at 0x00000000
 *            @arg SYSCFG_MemoryRemap_FSMC:        FSMC (Bank1 (NOR/PSRAM 1
 * and 2) mapped at 0x00000000 for STM32F405xx/407xx, STM32F415xx/417xx and
 * STM32F413_423xx devices.
 *            @arg SYSCFG_MemoryRemap_FMC:         FMC (Bank1 (NOR/PSRAM 1 and
 * 2) mapped at 0x00000000 for STM32F42xxx/43xxx devices.
 *            @arg SYSCFG_MemoryRemap_ExtMEM:      External Memory mapped at
 * 0x00000000 for STM32F446xx/STM32F469_479xx devices.
 *            @arg SYSCFG_MemoryRemap_SRAM:        Embedded SRAM (112kB)
 * mapped at 0x00000000
 *            @arg SYSCFG_MemoryRemap_SDRAM:       FMC (External SDRAM) mapped
 * at 0x00000000 for STM32F42xxx/43xxx devices.
 * @retval None
 */
...
/**
  ...
  *          This parameter can be one of the following values:
  *            @arg ENABLE: Flash Bank2 mapped at 0x08000000 (and aliased
  @0x00000000)
  *                         and Flash Bank1 mapped at 0x08100000 (and aliased
  at 0x00100000)
  *            @arg DISABLE:(the default state) Flash Bank1 mapped at
  0x08000000 (and aliased @0x0000 0000) and Flash Bank2 mapped at 0x08100000
 ...
 * for EXTI lines where x can be (A..K) for STM32F42xxx/43xxx devices, (A..I)
 *          for STM32F405xx/407xx and STM32F415xx/417xx devices or (A, B, C, D
 * and H) for STM32401xx devices.
 *
 * @param  EXTI_PinSourcex: specifies the EXTI line to be configured.
 *           This parameter can be EXTI_PinSourcex where x can be (0..15,
 * except for EXTI_PortSourceGPIOI x can be (0..11) for STM32F405xx/407xx and
 * STM32F405xx/407xx devices and for EXTI_PortSourceGPIOK x can be (0..7) for
 ...
 * @param  None
 * @retval The new state of the I/O Compensation Cell ready flag (SET or
 * RESET)
 */
 ...
 *     @arg SYSCFG_Break_PVD: PVD interrupt is connected to the break input of
 * TIM1/8.
 *     @arg SYSCFG_Break_HardFault: Lockup output of CortexM4 is connected to
 * the break input of TIM1/8.
 * @retval None
 */
...
void DFSDM_DataIn2_SourceSelection(uint32_t source) {
	...
	if ((source == DATAIN2_DFSDM2_PAD)
	    || (source == DATAIN2_DFSDM2_DATAIN3)) {
...
void DFSDM1_BitStreamClk_Config(uint32_t source) {
	...
	if ((source == DFSDM1_CLKIN0_TIM4OC2)
	    || (source == DFSDM1_CLKIN2_TIM4OC2)) {
...
void DFSDM2_BitStreamClk_Config(uint32_t source) {
	...
	if ((source == DFSDM2_CLKIN0_TIM3OC4)
	    || (source == DFSDM2_CLKIN4_TIM3OC4)) {
		tmp = (tmp & ~SYSCFG_MCHDLYCR_DFSDM2CK04SEL);
	} else if ((source == DFSDM2_CLKIN1_TIM3OC3)
	           || (source == DFSDM2_CLKIN5_TIM3OC3)) {
		tmp = (tmp & ~SYSCFG_MCHDLYCR_DFSDM2CK15SEL);

	} else if ((source == DFSDM2_CLKIN2_TIM3OC2)
	           || (source == DFSDM2_CLKIN6_TIM3OC2)) {
