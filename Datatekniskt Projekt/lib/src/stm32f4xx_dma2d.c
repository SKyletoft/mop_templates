...
/**
 * @brief  Initializes the DMA2D peripheral according to the specified
 * parameters in the DMA2D_InitStruct.
 * @note   This function can be used only when the DMA2D is disabled.
 * @param  DMA2D_InitStruct: pointer to a DMA2D_InitTypeDef structure that
 * contains the configuration information for the specified DMA2D peripheral.
 * @retval None
 */
void DMA2D_Init(DMA2D_InitTypeDef *DMA2D_InitStruct) {
	...
	assert_param(
	    IS_DMA2D_OUTPUT_OFFSET(DMA2D_InitStruct->DMA2D_OutputOffset));
	...
	DMA2D->OCOLR |= ((outgreen) | (outred)
	                 | (DMA2D_InitStruct->DMA2D_OutputBlue) | (outalpha));
	 ...
	 * @param  DMA2D_InitStruct: pointer to a DMA2D_InitTypeDef structure
 ...
 * @param  DMA2D_FGStruct: pointer to a DMA2D_FGTypeDef structure that
 * contains the configuration information for the specified Background.
 * @retval None
 */
void DMA2D_FGConfig(DMA2D_FG_InitTypeDef *DMA2D_FG_InitStruct) {
	...
	assert_param(
	    IS_DMA2D_FG_CLUT_SIZE(DMA2D_FG_InitStruct->DMA2D_FG_CLUT_SIZE));
	assert_param(
	    IS_DMA2D_FG_ALPHA_MODE(DMA2D_FG_InitStruct->DMA2D_FGPFC_ALPHA_MODE));
	assert_param(IS_DMA2D_FG_ALPHA_VALUE(
	...
	DMA2D->FGCOLR |=
	    (DMA2D_FG_InitStruct->DMA2D_FGC_BLUE | fg_colorgreen | fg_colorred);
 ...
 * @note   This function can be used only when the transfer is disabled.
 * @param  DMA2D_BGStruct: pointer to a DMA2D_BGTypeDef structure that
 * contains the configuration information for the specified Background.
 * @retval None
 */
void DMA2D_BGConfig(DMA2D_BG_InitTypeDef *DMA2D_BG_InitStruct) {
	...
	assert_param(
	    IS_DMA2D_BG_CLUT_SIZE(DMA2D_BG_InitStruct->DMA2D_BG_CLUT_SIZE));
	assert_param(
	    IS_DMA2D_BG_ALPHA_MODE(DMA2D_BG_InitStruct->DMA2D_BGPFC_ALPHA_MODE));
	assert_param(IS_DMA2D_BG_ALPHA_VALUE(
	...
	DMA2D->BGCOLR |=
	    (DMA2D_BG_InitStruct->DMA2D_BGC_BLUE | bg_colorgreen | bg_colorred);
 ...
 ===============================================================================
    ...
    *** Interrupts ***
    ...
    [..]
      ...
      (+) DMA2D_IT_TE: Transfer Error interrupt is generated when the CPU
trying to access the CLUT while a CLUT loading or a DMA2D1 transfer is on
going
 ...
 * @param  DMA2D_IT: specifies the DMA2D interrupts sources to be enabled or
...
ITStatus DMA2D_GetITStatus(uint32_t DMA2D_IT) {
	...
	if (((DMA2D->CR & DMA2D_IT) != (uint32_t) RESET)
