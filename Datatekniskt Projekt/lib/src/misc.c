/**
  ...
  *     NVIC_PriorityGroup_4  |                0-15               | 0 | 4 bits
  for pre-emption priority
  *                           |                                   | | 0 bits
  for subpriority
  *
  ==========================================================================================================================
  *
  *            2. Enable and Configure the priority of the selected IRQ
  Channels using NVIC_Init()
  *
 ...
 * @note   When the NVIC_PriorityGroup_0 is selected, IRQ pre-emption is no
 * more possible. The pending IRQ priority will be managed only by the
 * subpriority.
 * @retval None
 */
...
void NVIC_Init(NVIC_InitTypeDef *NVIC_InitStruct) {
	...
	if (NVIC_InitStruct->NVIC_IRQChannelCmd != DISABLE) {
		...
		tmppriority = NVIC_InitStruct->NVIC_IRQChannelPreemptionPriority
		           << tmppre;
		tmppriority |=
