...
@verbatim
 ...
 [..] The following sequence should be followed to configure the FMC to
interface with SRAM, PSRAM, NOR or OneNAND memory connected to the NOR/SRAM
Bank:

   (#) Enable the clock for the FMC and associated GPIOs using the following
functions: RCC_AHB3PeriphClockCmd(RCC_AHB3Periph_FMC, ENABLE);
   ...
   (#) FMC pins configuration
       ...
       (++) Configure these FMC pins in alternate function mode by calling the
function GPIO_Init();

   (#) Declare a FMC_NORSRAMInitTypeDef structure, for example:
          FMC_NORSRAMInitTypeDef  FMC_NORSRAMInitStructure;
      and fill the FMC_NORSRAMInitStructure variable with the allowed values
...
void FMC_NORSRAMInit(FMC_NORSRAMInitTypeDef *FMC_NORSRAMInitStruct) {
	...
	assert_param(IS_FMC_NORSRAM_MEMORY_WIDTH(
	    FMC_NORSRAMInitStruct->FMC_MemoryDataWidth));
	assert_param(
	    IS_FMC_BURSTMODE(FMC_NORSRAMInitStruct->FMC_BurstAccessMode));
	assert_param(
	    IS_FMC_WAIT_POLARITY(FMC_NORSRAMInitStruct->FMC_WaitSignalPolarity));
	assert_param(IS_FMC_WRAP_MODE(FMC_NORSRAMInitStruct->FMC_WrapMode));
	assert_param(IS_FMC_WAIT_SIGNAL_ACTIVE(
	    FMC_NORSRAMInitStruct->FMC_WaitSignalActive));
	assert_param(
	    IS_FMC_WRITE_OPERATION(FMC_NORSRAMInitStruct->FMC_WriteOperation));
	assert_param(IS_FMC_WAITE_SIGNAL(FMC_NORSRAMInitStruct->FMC_WaitSignal));
	assert_param(
	    IS_FMC_EXTENDED_MODE(FMC_NORSRAMInitStruct->FMC_ExtendedMode));
	assert_param(
	    IS_FMC_ASYNWAIT(FMC_NORSRAMInitStruct->FMC_AsynchronousWait));
	assert_param(IS_FMC_WRITE_BURST(FMC_NORSRAMInitStruct->FMC_WriteBurst));
	assert_param(
	    IS_FMC_CONTINOUS_CLOCK(FMC_NORSRAMInitStruct->FMC_ContinousClock));
	...
	assert_param(IS_FMC_CLK_DIV(
	    FMC_NORSRAMInitStruct->FMC_ReadWriteTimingStruct->FMC_CLKDivision));
	...
	/* Clear MBKEN, MUXEN, MTYP, MWID, FACCEN, BURSTEN, WAITPOL, WRAPMOD,
	   WAITCFG, WREN, WAITEN, EXTMOD, ASYNCWAIT, CBURSTRW and CCLKEN bits */
	tmpbcr &= ((uint32_t) ~(
	...
	if ((FMC_NORSRAMInitStruct->FMC_ContinousClock == FMC_CClock_SyncAsync)
		...
		FMC_Bank1->BTCR[FMC_Bank1_NORSRAM1] |=
		    FMC_NORSRAMInitStruct->FMC_ContinousClock;
	...
	FMC_Bank1->BTCR[FMC_NORSRAMInitStruct->FMC_Bank + 1] =
	    (uint32_t) FMC_NORSRAMInitStruct->FMC_ReadWriteTimingStruct
	        ->FMC_AddressSetupTime
	    | (FMC_NORSRAMInitStruct->FMC_ReadWriteTimingStruct
	           ->FMC_AddressHoldTime
	       << 4)
	...
	/* NOR/SRAM Bank timing register for write configuration, if extended mode
	 * is used */
	if (FMC_NORSRAMInitStruct->FMC_ExtendedMode == FMC_ExtendedMode_Enable) {
		...
		tmpbwr &= ((uint32_t) ~(FMC_BWTR1_ADDSET | FMC_BWTR1_ADDHLD
		                        | FMC_BWTR1_DATAST | FMC_BWTR1_BUSTURN
		                        | FMC_BWTR1_ACCMOD));

		tmpbwr |= (uint32_t) (FMC_NORSRAMInitStruct->FMC_WriteTimingStruct
		                          ->FMC_AddressSetupTime
		                      | (FMC_NORSRAMInitStruct->FMC_WriteTimingStruct
		                             ->FMC_AddressHoldTime
		                         << 4)
...
void FMC_NORSRAMStructInit(FMC_NORSRAMInitTypeDef *FMC_NORSRAMInitStruct) {
	...
	FMC_NORSRAMInitStruct->FMC_MemoryDataWidth =
	    FMC_NORSRAM_MemoryDataWidth_16b;
	...
	FMC_NORSRAMInitStruct->FMC_ReadWriteTimingStruct =
	    (FMC_NORSRAMTimingInitTypeDef *) ((
	        uint32_t) &FMC_DefaultTimingStruct);
	FMC_NORSRAMInitStruct->FMC_WriteTimingStruct =
	    (FMC_NORSRAMTimingInitTypeDef *) ((
	        uint32_t) &FMC_DefaultTimingStruct);
}
 ...
 *            @arg FMC_Bank1_NORSRAM3: FMC Bank1 NOR/SRAM3
 *            @arg FMC_Bank1_NORSRAM4: FMC Bank1 NOR/SRAM4
 * @param  NewState: new state of the FMC_Bank. This parameter can be: ENABLE
 * or DISABLE.
 * @retval None
 */
void FMC_NORSRAMCmd(uint32_t FMC_Bank, FunctionalState NewState) {
	...
	if (NewState != DISABLE) {
		...
		/* Disable the selected NOR/SRAM Bank by clearing the PBKEN bit in the
		 * BCRx register */
...
@verbatim
 ...
 [..]  The following sequence should be followed to configure the FMC to
interface with 8-bit or 16-bit NAND memory connected to the NAND Bank:
...
/**
 * @brief  De-initializes the FMC NAND Banks registers to their default reset
 * values.
 * @param  FMC_Bank: specifies the FMC Bank to be used
 *          This parameter can be one of the following values:
 ...
 * @param  FMC_NANDInitStruct : pointer to a FMC_NANDInitTypeDef structure
 * that contains the configuration information for the FMC NAND specified
 * Banks.
 * @retval None
 */
void FMC_NANDInit(FMC_NANDInitTypeDef *FMC_NANDInitStruct) {
	...
	assert_param(
	    IS_FMC_NAND_MEMORY_WIDTH(FMC_NANDInitStruct->FMC_MemoryDataWidth));
	...
	assert_param(IS_FMC_SETUP_TIME(
	    FMC_NANDInitStruct->FMC_CommonSpaceTimingStruct->FMC_SetupTime));
	...
	assert_param(IS_FMC_HIZ_TIME(
	    FMC_NANDInitStruct->FMC_CommonSpaceTimingStruct->FMC_HiZSetupTime));
	assert_param(IS_FMC_SETUP_TIME(
	    FMC_NANDInitStruct->FMC_AttributeSpaceTimingStruct->FMC_SetupTime));
	assert_param(
	    IS_FMC_WAIT_TIME(FMC_NANDInitStruct->FMC_AttributeSpaceTimingStruct
	                         ->FMC_WaitSetupTime));
	assert_param(
	    IS_FMC_HOLD_TIME(FMC_NANDInitStruct->FMC_AttributeSpaceTimingStruct
	                         ->FMC_HoldSetupTime));
	assert_param(
	    IS_FMC_HIZ_TIME(FMC_NANDInitStruct->FMC_AttributeSpaceTimingStruct
	                        ->FMC_HiZSetupTime));
	...
	tmppcr |= (uint32_t) FMC_NANDInitStruct->FMC_Waitfeature
	        | PCR_MEMORYTYPE_NAND | FMC_NANDInitStruct->FMC_MemoryDataWidth
	...
	tmppmem &= ((uint32_t) ~(FMC_PMEM2_MEMSET2 | FMC_PMEM2_MEMWAIT2
	                         | FMC_PMEM2_MEMHOLD2 | FMC_PMEM2_MEMHIZ2));

	/* Set tmppmem value according to FMC_CommonSpaceTimingStructure
	 * parameters */
	tmppmem |=
	    (uint32_t)
	        FMC_NANDInitStruct->FMC_CommonSpaceTimingStruct->FMC_SetupTime
	...
	tmppatt &= ((uint32_t) ~(FMC_PATT2_ATTSET2 | FMC_PATT2_ATTWAIT2
	                         | FMC_PATT2_ATTHOLD2 | FMC_PATT2_ATTHIZ2));

	/* Set tmppatt value according to FMC_AttributeSpaceTimingStructure
	 * parameters */
	tmppatt |=
	    (uint32_t)
	        FMC_NANDInitStruct->FMC_AttributeSpaceTimingStruct->FMC_SetupTime
	    | (FMC_NANDInitStruct->FMC_AttributeSpaceTimingStruct
	           ->FMC_WaitSetupTime
	       << 8)
	    | (FMC_NANDInitStruct->FMC_AttributeSpaceTimingStruct
	           ->FMC_HoldSetupTime
...
@verbatim
 ...
 [..]  he following sequence should be followed to configure the FMC to
interface with 16-bit PC Card compatible memory connected to the PCCARD Bank:
...
/**
 * @brief  De-initializes the FMC PCCARD Bank registers to their default reset
 * values.
 * @param  None
 * @retval None
 */
...
/**
 * @brief  Initializes the FMC PCCARD Bank according to the specified
 * parameters in the FMC_PCCARDInitStruct.
 * @param  FMC_PCCARDInitStruct : pointer to a FMC_PCCARDInitTypeDef structure
 *         that contains the configuration information for the FMC PCCARD
 * Bank.
 * @retval None
 */
void FMC_PCCARDInit(FMC_PCCARDInitTypeDef *FMC_PCCARDInitStruct) {
	...
	assert_param(
	    IS_FMC_WAIT_TIME(FMC_PCCARDInitStruct->FMC_CommonSpaceTimingStruct
	                         ->FMC_WaitSetupTime));
	assert_param(
	    IS_FMC_HOLD_TIME(FMC_PCCARDInitStruct->FMC_CommonSpaceTimingStruct
	                         ->FMC_HoldSetupTime));
	assert_param(IS_FMC_HIZ_TIME(
	...
	assert_param(
	    IS_FMC_WAIT_TIME(FMC_PCCARDInitStruct->FMC_AttributeSpaceTimingStruct
	                         ->FMC_WaitSetupTime));
	assert_param(
	    IS_FMC_HOLD_TIME(FMC_PCCARDInitStruct->FMC_AttributeSpaceTimingStruct
	                         ->FMC_HoldSetupTime));
	assert_param(
	    IS_FMC_HIZ_TIME(FMC_PCCARDInitStruct->FMC_AttributeSpaceTimingStruct
	                        ->FMC_HiZSetupTime));
	assert_param(IS_FMC_SETUP_TIME(
	    FMC_PCCARDInitStruct->FMC_IOSpaceTimingStruct->FMC_SetupTime));
	assert_param(IS_FMC_WAIT_TIME(
	    FMC_PCCARDInitStruct->FMC_IOSpaceTimingStruct->FMC_WaitSetupTime));
	assert_param(IS_FMC_HOLD_TIME(
	    FMC_PCCARDInitStruct->FMC_IOSpaceTimingStruct->FMC_HoldSetupTime));
	assert_param(IS_FMC_HIZ_TIME(
	    FMC_PCCARDInitStruct->FMC_IOSpaceTimingStruct->FMC_HiZSetupTime));
	...
	/* Set the PCR4 register value according to FMC_PCCARDInitStruct
	 * parameters */
	...
	/* Clear MEMSETx, MEMWAITx, MEMHOLDx and MEMHIZx bits */
	tmpmem &= ((uint32_t) ~(FMC_PMEM4_MEMSET4 | FMC_PMEM4_MEMWAIT4
	                        | FMC_PMEM4_MEMHOLD4 | FMC_PMEM4_MEMHIZ4));

	/* Set PMEM4 register value according to FMC_CommonSpaceTimingStructure
	 * parameters */
	tmpmem |=
	    (uint32_t)
	        FMC_PCCARDInitStruct->FMC_CommonSpaceTimingStruct->FMC_SetupTime
	    | (FMC_PCCARDInitStruct->FMC_CommonSpaceTimingStruct
	           ->FMC_WaitSetupTime
	       << 8)
	    | (FMC_PCCARDInitStruct->FMC_CommonSpaceTimingStruct
	           ->FMC_HoldSetupTime
	...
	/* Clear ATTSETx, ATTWAITx, ATTHOLDx and ATTHIZx bits */
	tmppatt &= ((uint32_t) ~(FMC_PATT4_ATTSET4 | FMC_PATT4_ATTWAIT4
	                         | FMC_PATT4_ATTHOLD4 | FMC_PATT4_ATTHIZ4));

	/* Set PATT4 register value according to FMC_AttributeSpaceTimingStructure
	 * parameters */
	tmppatt |= (uint32_t) FMC_PCCARDInitStruct->FMC_AttributeSpaceTimingStruct
	               ->FMC_SetupTime
	         | (FMC_PCCARDInitStruct->FMC_AttributeSpaceTimingStruct
	                ->FMC_WaitSetupTime
	            << 8)
	         | (FMC_PCCARDInitStruct->FMC_AttributeSpaceTimingStruct
	                ->FMC_HoldSetupTime
	...
	tmppio &= ((uint32_t) ~(FMC_PIO4_IOSET4 | FMC_PIO4_IOWAIT4
	                        | FMC_PIO4_IOHOLD4 | FMC_PIO4_IOHIZ4));

	/* Set PIO4 register value according to FMC_IOSpaceTimingStructure
	 * parameters */
	tmppio |=
	    (uint32_t)
	        FMC_PCCARDInitStruct->FMC_IOSpaceTimingStruct->FMC_SetupTime
...
@verbatim
 ...
 [..]  The following sequence should be followed to configure the FMC to
interface with SDRAM memory connected to the SDRAM Bank 1 or SDRAM bank 2:
...
function GPIO_Init();
  ...
  (#) Declare a FMC_SDRAMCommandTypeDef structure, for example:
      ...
      and fill the FMC_SDRAMCommandStructure variable with the allowed values
of the structure member.

  (#) Configure the SDCMR register with the desired command parameters by
calling the function FMC_SDRAMCmdConfig(&FMC_SDRAMCommandStructure);
...
void FMC_SDRAMInit(FMC_SDRAMInitTypeDef *FMC_SDRAMInitStruct) {
	...
	assert_param(
	    IS_FMC_COLUMNBITS_NUMBER(FMC_SDRAMInitStruct->FMC_ColumnBitsNumber));
	assert_param(
	    IS_FMC_ROWBITS_NUMBER(FMC_SDRAMInitStruct->FMC_RowBitsNumber));
	assert_param(
	    IS_FMC_SDMEMORY_WIDTH(FMC_SDRAMInitStruct->FMC_SDMemoryDataWidth));
	assert_param(IS_FMC_INTERNALBANK_NUMBER(
	    FMC_SDRAMInitStruct->FMC_InternalBankNumber));
	assert_param(IS_FMC_CAS_LATENCY(FMC_SDRAMInitStruct->FMC_CASLatency));
	assert_param(
	    IS_FMC_WRITE_PROTECTION(FMC_SDRAMInitStruct->FMC_WriteProtection));
	assert_param(
	    IS_FMC_SDCLOCK_PERIOD(FMC_SDRAMInitStruct->FMC_SDClockPeriod));
	assert_param(IS_FMC_READ_BURST(FMC_SDRAMInitStruct->FMC_ReadBurst));
	assert_param(
	    IS_FMC_READPIPE_DELAY(FMC_SDRAMInitStruct->FMC_ReadPipeDelay));
	...
	assert_param(IS_FMC_EXITSELFREFRESH_DELAY(
	    FMC_SDRAMInitStruct->FMC_SDRAMTimingStruct
	...
	assert_param(IS_FMC_RP_DELAY(
	    FMC_SDRAMInitStruct->FMC_SDRAMTimingStruct->FMC_RPDelay));
	assert_param(IS_FMC_RCD_DELAY(
	    FMC_SDRAMInitStruct->FMC_SDRAMTimingStruct->FMC_RCDDelay));
	...
	if (FMC_SDRAMInitStruct->FMC_Bank == FMC_Bank1_SDRAM) {
		...
		tmpr2 &= ((uint32_t) ~(
		    FMC_SDTR1_TMRD | FMC_SDTR1_TXSR | FMC_SDTR1_TRAS | FMC_SDTR1_TRC
		    | FMC_SDTR1_TWR | FMC_SDTR1_TRP | FMC_SDTR1_TRCD));

		tmpr2 |=
		    (uint32_t) ((FMC_SDRAMInitStruct->FMC_SDRAMTimingStruct
		    ...
		    | (((FMC_SDRAMInitStruct->FMC_SDRAMTimingStruct
		        ...
		        - 1)
		       << 16)
		    | (((FMC_SDRAMInitStruct->FMC_SDRAMTimingStruct->FMC_RPDelay) - 1)
		       << 20)
		    | (((FMC_SDRAMInitStruct->FMC_SDRAMTimingStruct->FMC_RCDDelay)
		        - 1)
		       << 24);
...
/**
 * @brief  Configures the SDRAM memory command issued when the device is
 * accessed.
 * @param  FMC_SDRAMCommandStruct: pointer to a FMC_SDRAMCommandTypeDef
 * structure which will be configured.
 * @retval None
 */
void FMC_SDRAMCmdConfig(FMC_SDRAMCommandTypeDef *FMC_SDRAMCommandStruct) {
	...
	/* check parameters */
	assert_param(
	    IS_FMC_COMMAND_MODE(FMC_SDRAMCommandStruct->FMC_CommandMode));
	assert_param(
	    IS_FMC_COMMAND_TARGET(FMC_SDRAMCommandStruct->FMC_CommandTarget));
	assert_param(IS_FMC_AUTOREFRESH_NUMBER(
	    FMC_SDRAMCommandStruct->FMC_AutoRefreshNumber));
	assert_param(IS_FMC_MODE_REGISTER(
	    FMC_SDRAMCommandStruct->FMC_ModeRegisterDefinition));

	tmpr =
	    (uint32_t) (FMC_SDRAMCommandStruct->FMC_CommandMode
...
/**
 * @brief  Enables or disables write protection to the specified FMC SDRAM
 * Bank.
 * @param  SDRAM_Bank: Defines the FMC SDRAM bank. This parameter can be
 *                     FMC_Bank1_SDRAM or FMC_Bank2_SDRAM.
 ...
 *            @arg FMC_Bank1_SDRAM: FMC Bank1 SDRAM
 *            @arg FMC_Bank2_SDRAM: FMC Bank2 SDRAM
 * @param  FMC_IT: specifies the FMC interrupt sources to be enabled or
