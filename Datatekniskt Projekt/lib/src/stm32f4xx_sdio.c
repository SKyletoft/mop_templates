/**
  ...
  *          functionalities of the Secure digital input/output interface
(SDIO)
  *          peripheral:
...
@verbatim
 ...
 ===================================================================
 [..]
   (#) The SDIO clock (SDIOCLK = 48 MHz) is coming from a specific output of
PLL (PLL48CLK). Before to start working with SDIO peripheral make sure that
the PLL is well configured. The SDIO peripheral uses two clock signals:
       (++) SDIO adapter clock (SDIOCLK = 48 MHz)
       (++) APB2 bus clock (PCLK2)

       -@@- PCLK2 and SDIO_CK clock frequencies must respect the following
condition: Frequency(PCLK2) >= (3 / 8 x Frequency(SDIO_CK))
...
SDIO_SetPowerState(SDIO_PowerState_ON) function.
...
has to fill the command structure (pointer to SDIO_CmdInitTypeDef) according
...
#define CMD_NIEN_BB                                                          \
	(PERIPH_BB_BASE + (CMD_OFFSET * 32) + (NIEN_BitNumber * 4))
 ...
 * @param  None
 * @retval Power status of the controller. The returned value can be one of
...
/** @defgroup SDIO_Group2 Command path state machine (CPSM) management
functions
...
/**
 * @brief  Returns the number of words left to be written to or read from
 * FIFO.
 * @param  None
 ...
 *            @arg SDIO_ReadWaitMode_CLK: Read Wait control by stopping
 * SDIOCLK
 ...
 * disabled. This parameter can be one or a combination of the following
 * values:
 ...
 * zero) interrupt
 *            @arg SDIO_IT_STBITERR: Start bit not detected on all data
