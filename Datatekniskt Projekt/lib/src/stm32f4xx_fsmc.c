...
functions: RCC_AHB3PeriphClockCmd(RCC_AHB3Periph_FSMC, ENABLE);
          RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOx, ENABLE);

   (#) FSMC pins configuration
       (++) Connect the involved FSMC pins to AF12 using the following
function GPIO_PinAFConfig(GPIOx, GPIO_PinSourcex, GPIO_AF_FSMC);
       (++) Configure these FSMC pins in alternate function mode by calling
the function GPIO_Init();

   (#) Declare a FSMC_NORSRAMInitTypeDef structure, for example:
          FSMC_NORSRAMInitTypeDef  FSMC_NORSRAMInitStructure;
      and fill the FSMC_NORSRAMInitStructure variable with the allowed values
...
void FSMC_NORSRAMInit(FSMC_NORSRAMInitTypeDef *FSMC_NORSRAMInitStruct) {
	...
	tmpbcr &= ((uint32_t) ~(
	    ...
	    | FSMC_BCR1_FACCEN | FSMC_BCR1_BURSTEN | FSMC_BCR1_WAITPOL
	    | FSMC_BCR1_WRAPMOD | FSMC_BCR1_WAITCFG | FSMC_BCR1_WREN
	 ...
	 */
	tmpbtr &=
	    ((uint32_t) ~(FSMC_BTR1_ADDSET | FSMC_BTR1_ADDHLD | FSMC_BTR1_DATAST
	                  | FSMC_BTR1_BUSTURN | FSMC_BTR1_CLKDIV
	...
	if (FSMC_NORSRAMInitStruct->FSMC_ExtendedMode
		...
		tmpbwr &= ((uint32_t) ~(FSMC_BWTR1_ADDSET | FSMC_BWTR1_ADDHLD
...
void FSMC_NORSRAMStructInit(FSMC_NORSRAMInitTypeDef *FSMC_NORSRAMInitStruct) {
	...
	FSMC_NORSRAMInitStruct->FSMC_ReadWriteTimingStruct =
	    (FSMC_NORSRAMTimingInitTypeDef *) ((
	        uint32_t) &FSMC_DefaultTimingStruct);
	FSMC_NORSRAMInitStruct->FSMC_WriteTimingStruct =
	    (FSMC_NORSRAMTimingInitTypeDef *) ((
	        uint32_t) &FSMC_DefaultTimingStruct);
}
 ...
 * that contains the configuration information for the FSMC NAND specified
 * Banks.
 * @retval None
 */
void FSMC_NANDInit(FSMC_NANDInitTypeDef *FSMC_NANDInitStruct) {
	...
	assert_param(
	    IS_FSMC_WAIT_TIME(FSMC_NANDInitStruct->FSMC_CommonSpaceTimingStruct
	                          ->FSMC_WaitSetupTime));
	assert_param(
	    IS_FSMC_HOLD_TIME(FSMC_NANDInitStruct->FSMC_CommonSpaceTimingStruct
	                          ->FSMC_HoldSetupTime));
	assert_param(
	    IS_FSMC_HIZ_TIME(FSMC_NANDInitStruct->FSMC_CommonSpaceTimingStruct
	                         ->FSMC_HiZSetupTime));
	assert_param(IS_FSMC_SETUP_TIME(
	...
	assert_param(
	    IS_FSMC_HOLD_TIME(FSMC_NANDInitStruct->FSMC_AttributeSpaceTimingStruct
	                          ->FSMC_HoldSetupTime));
	assert_param(
	    IS_FSMC_HIZ_TIME(FSMC_NANDInitStruct->FSMC_AttributeSpaceTimingStruct
	                         ->FSMC_HiZSetupTime));
	...
	tmppcr &=
	    ((uint32_t) ~(FSMC_PCR2_PWAITEN | FSMC_PCR2_PBKEN | FSMC_PCR2_PTYP
	...
	tmppmem &= ((uint32_t) ~(FSMC_PMEM2_MEMSET2 | FSMC_PMEM2_MEMWAIT2
	...
	tmppatt &= ((uint32_t) ~(FSMC_PATT2_ATTSET2 | FSMC_PATT2_ATTWAIT2
	                         | FSMC_PATT2_ATTHOLD2 | FSMC_PATT2_ATTHIZ2));

	/* Set tmppatt value according to FSMC_AttributeSpaceTimingStructure
	 * parameters */
	tmppatt |= (uint32_t) FSMC_NANDInitStruct->FSMC_AttributeSpaceTimingStruct
	               ->FSMC_SetupTime
	         | (FSMC_NANDInitStruct->FSMC_AttributeSpaceTimingStruct
	                ->FSMC_WaitSetupTime
	            << 8)
...
the function GPIO_Init();
  ...
  (#) Declare a FSMC_PCCARDInitTypeDef structure, for example:
      ...
      and fill the FSMC_PCCARDInitStructure variable with the allowed values
of the structure member.
...
/**
 * @brief  De-initializes the FSMC PCCARD Bank registers to their default
 * reset values.
 * @param  None
 * @retval None
 */
 ...
 * structure that contains the configuration information for the FSMC PCCARD
 * Bank.
 * @retval None
 */
void FSMC_PCCARDInit(FSMC_PCCARDInitTypeDef *FSMC_PCCARDInitStruct) {
	...
	assert_param(
	    IS_FSMC_TCLR_TIME(FSMC_PCCARDInitStruct->FSMC_TCLRSetupTime));
	assert_param(IS_FSMC_TAR_TIME(FSMC_PCCARDInitStruct->FSMC_TARSetupTime));

	assert_param(IS_FSMC_SETUP_TIME(
	    FSMC_PCCARDInitStruct->FSMC_CommonSpaceTimingStruct->FSMC_SetupTime));
	assert_param(
	    IS_FSMC_WAIT_TIME(FSMC_PCCARDInitStruct->FSMC_CommonSpaceTimingStruct
	                          ->FSMC_WaitSetupTime));
	assert_param(
	    IS_FSMC_HOLD_TIME(FSMC_PCCARDInitStruct->FSMC_CommonSpaceTimingStruct
	                          ->FSMC_HoldSetupTime));
	assert_param(
	    IS_FSMC_HIZ_TIME(FSMC_PCCARDInitStruct->FSMC_CommonSpaceTimingStruct
	                         ->FSMC_HiZSetupTime));
	...
	tmppcr4 &= ((uint32_t) ~(FSMC_PCR4_TAR | FSMC_PCR4_TCLR
	                         | FSMC_PCR4_PWAITEN | FSMC_PCR4_PWID));
	...
	/* Clear MEMSETx, MEMWAITx, MEMHOLDx and MEMHIZx bits */
	tmppmem4 &= ((uint32_t) ~(FSMC_PMEM4_MEMSET4 | FSMC_PMEM4_MEMWAIT4
	...
	tmppio4 &= ((uint32_t) ~(FSMC_PIO4_IOSET4 | FSMC_PIO4_IOWAIT4
