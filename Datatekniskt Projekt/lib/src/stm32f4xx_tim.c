 ...
 * structure that contains the configuration information for the specified TIM
 * peripheral.
 * @retval None
 */
void TIM_TimeBaseInit(TIM_TypeDef *TIMx,
	...
	assert_param(IS_TIM_ALL_PERIPH(TIMx));
	assert_param(
	    IS_TIM_COUNTER_MODE(TIM_TimeBaseInitStruct->TIM_CounterMode));
...
driver,
        (++) if needed, else the Timer will run with the default
configuration: Autoreload value = 0xFFFF
...
function TIM_ITConfig(TIMx, TIM_IT_CCx) (or TIM_DMA_Cmd(TIMx, TIM_DMA_CCx))
...
void TIM_OC1Init(TIM_TypeDef *TIMx, TIM_OCInitTypeDef *TIM_OCInitStruct) {
	...
	if ((TIMx == TIM1) || (TIMx == TIM8)) {
		assert_param(
		    IS_TIM_OUTPUTN_STATE(TIM_OCInitStruct->TIM_OutputNState));
		assert_param(IS_TIM_OCN_POLARITY(TIM_OCInitStruct->TIM_OCNPolarity));
		assert_param(
		    IS_TIM_OCNIDLE_STATE(TIM_OCInitStruct->TIM_OCNIdleState));
 ...
 * Output Compare Mode. If needed, user has to enable this channel using
...
channel with the corresponding configuration and to measure only frequency or
duty cycle of the input signal, or, Call TIM_PWMIConfig(TIMx,
&TIM_ICInitStruct) to configure the desired channels with the corresponding
configuration and to measure the frequency and the duty cycle of the input
signal

      (#) Enable the NVIC or the DMA to read the measured frequency.

      (#) Enable the corresponding interrupt (or DMA request) to read the
Captured value, using the function TIM_ITConfig(TIMx, TIM_IT_CCx) (or
...
mode:
      ...
      (#) Fill the TIM_BDTRInitStruct with the desired parameters for the
Timer Break Polarity, dead time, Lock level, the OSSI/OSSR State and the
          AOE(automatic output enable).

      (#) Call TIM_BDTRConfig(TIMx, &TIM_BDTRInitStruct) to configure the
Timer
...
reset state or in a known state (according to the configuration made in
...
void TIM_BDTRConfig(TIM_TypeDef *TIMx,
	...
	assert_param(
	    IS_TIM_BREAK_POLARITY(TIM_BDTRInitStruct->TIM_BreakPolarity));
 ...
 * @note   For TIM9 and TIM12 only one of the following parameters can be
 * used: TIM_IT_Update, TIM_IT_CC1, TIM_IT_CC2 or TIM_IT_Trigger.
...
TIM_ExtTRGPrescaler, uint16_t TIM_ExtTRGPolarity, uint16_t ExtTRGFilter);
      (#) Configure the Slave Timers using the following functions:
        (++) void TIM_SelectInputTrigger(TIM_TypeDef* TIMx, uint16_t
TIM_InputTriggerSource);
        (++) void TIM_SelectSlaveMode(TIM_TypeDef* TIMx, uint16_t
 ...
 *
 *  - For all TIMx
 *            @arg TIM_TRGOSource_Reset:  The UG bit in the TIM_EGR register
 * is used as the trigger output(TRGO)
 *            @arg TIM_TRGOSource_Enable: The Counter Enable CEN is used as
 ...
 * pulse when the CC1IF flag is to be set, as soon as a capture or compare
 * match occurs(TRGO)
 ...
 * signal(TRGI) reinitialize the counter and triggers an update of the
 * registers
 ...
 *          This parameter can be one of the following values:
 *            @arg TIM2_TIM8_TRGO: TIM2 ITR1 input is connected to TIM8
 ...
 * clock (HSE divided by a programmable prescaler)
 * @retval None
 */
