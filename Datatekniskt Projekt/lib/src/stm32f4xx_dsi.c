 ...
 *         values.
 * @param  DSIx: To select the DSIx peripheral, where x can be the different
 * DSI instances
 * @retval None
 */
...
/**
 * @brief Deinitialize the DSIx peripheral registers to their default reset
 * values.
 * @param DSIx: To select the DSIx peripheral, where x can be the different
 * DSI instances
 * @param DSI_InitStruct: pointer to a DSI_InitTypeDef structure that
 *        contains the configuration information for the specified DSI
 * peripheral.
 ...
 * @retval None
 */
void DSI_Init(DSI_TypeDef *DSIx, DSI_InitTypeDef *DSI_InitStruct,
	...
	assert_param(IS_DSI_AUTO_CLKLANE_CONTROL(
	    DSI_InitStruct->AutomaticClockLaneControl));
	...
	DSIx->WRPCR |= (((PLLInit->PLLNDIV) << 2) | ((PLLInit->PLLIDF) << 11)
	                | ((PLLInit->PLLODF) << 16));
 ...
 * @param DSI_InitStruct: pointer to a DSI_InitTypeDef structure which will be
 * initialized.
 * @retval None
 */
...
void DSI_ConfigVideoMode(DSI_TypeDef *DSIx, DSI_VidCfgTypeDef *VidCfg) {
	...
	/* Set the total line time (HLINE=HSA+HBP+HACT+HFP) in lane byte clock
	 * cycles */
...
/**
 * @brief  Select adapted command mode and configure the corresponding
 * parameters
 * @param  DSIx: To select the DSIx peripheral, where x can be the different
 * DSI instances
 ...
 * @retval None
 */
void DSI_ConfigAdaptedCommandMode(DSI_TypeDef *DSIx,
	...
	/* Configure the tearing effect source and polarity and select the refresh
	 * mode */
	DSIx->WCFGR &=
	    ~(DSI_WCFGR_TESRC | DSI_WCFGR_TEPOL | DSI_WCFGR_AR | DSI_WCFGR_VSPOL);
	DSIx->WCFGR |=
...
void DSI_ConfigCommand(DSI_TypeDef *DSIx, DSI_LPCmdTypeDef *LPCmd) {
	...
	DSIx->CMCR &= ~(DSI_CMCR_GSW0TX | DSI_CMCR_GSW1TX | DSI_CMCR_GSW2TX
	...
	DSIx->CMCR |= (LPCmd->LPGenShortWriteNoP | LPCmd->LPGenShortWriteOneP
...
void DSI_ConfigPhyTimer(DSI_TypeDef *DSIx, DSI_PHY_TimerTypeDef *PhyTimers) {
	...
	/* In Automatic Clock Lane control mode, the DSI Host can turn off the
	   clock lane between two High-Speed transmission. To do so, the DSI Host
	   calculates the time required for the clock lane to change from
	   HighSpeed to Low-Power and from Low-Power to High-Speed. This timings
	   are configured by the HS2LP_TIME and LP2HS_TIME in the DSI Host Clock
	   Lane Timer Configuration Register (DSI_CLTCR). But the DSI Host is not
	   calculating LP2HS_TIME + HS2LP_TIME but 2 x HS2LP_TIME.

	   Workaround : Configure HS2LP_TIME and LP2HS_TIME with the same value
	   being the max of HS2LP_TIME or LP2HS_TIME.
	*/
	...
	DSIx->DLTCR |= (PhyTimers->DataLaneMaxReadTime
	                | ((PhyTimers->DataLaneLP2HSTime) << 16)
 ...
 * @retval None
 */
void DSI_ConfigHostTimeouts(DSI_TypeDef *DSIx,
 ...
 * @param  DSIx: To select the DSIx peripheral, where x can be the different
 * DSI instances the configuration information for the DSI.
 * @retval None
 */
 ...
 *                 This parameter can be any value of @ref DSI_DCS_Command or
 * a generic command code.
 * @param  Param2: DSC parameter or second generic parameter.
 * @retval None
 */
void DSI_ShortWrite(DSI_TypeDef *DSIx, uint32_t ChannelID, uint32_t Mode,
 ...
 * @retval None
 */
void DSI_LongWrite(DSI_TypeDef *DSIx, uint32_t ChannelID, uint32_t Mode,
	...
	/* Set the DCS code hexadecimal on payload byte 1, and the other
	 * parameters on the write FIFO command*/
 ...
 * @param  Array: pointer to a buffer to store the payload of a read back
 * operation.
 ...
 * @retval None
 */
void DSI_Read(DSI_TypeDef *DSIx, uint32_t ChannelNbr, uint8_t *Array,
	...
	if (Size > 2) {
		...
		DSI_ShortWrite(DSIx, ChannelNbr, DSI_MAX_RETURN_PKT_SIZE,
...
void DSI_EnterULPM(DSI_TypeDef *DSIx) {
	...
	/* Use system PLL as byte lane clock source before stopping DSIPHY clock
	 * source */
 ...
 * @retval None
 */
void DSI_PatternGeneratorStart(DSI_TypeDef *DSIx, uint32_t Mode,
 ...
 * @retval None
 */
void DSI_SetSlewRateAndDelayTuning(DSI_TypeDef *DSIx, uint32_t CommDelay,
...
/**
 * @brief  Activate an additional current path on all lanes to meet the SDDTx
 ...
 * @retval None
 */
void DSI_SetLanePinsConfiguration(DSI_TypeDef *DSIx, uint32_t CustomLane,
 ...
 * @retval None
 */
void DSI_SetPHYTimings(DSI_TypeDef *DSIx, uint32_t Timing,
 ...
 * @retval None
 */
void DSI_ForceTXStopMode(DSI_TypeDef *DSIx, uint32_t Lane,
...
/**
 * @brief  Enable a pull-down on the lanes to prevent from floating states
 * when unused
 * @param  hdsi: pointer to a DSI_HandleTypeDef structure that contains
 *               the configuration information for the DSI.
 * @param  State: ENABLE or DISABLE
...
@verbatim
 ...
 [..] This section provides a set of functions allowing to configure the DSI
...
communication: Polling mode or Interrupt mode.
...
sources and 7 pending bits:
 ...
 [..] In this Mode it is advised to use the following functions:
   (+) void DSI_ITConfig(DSI_TypeDef* DSIx, uint32_t DSI_IT, FunctionalState
NewState);
 ...
 * @param DSIx: To select the DSIx peripheral, where x can be the different
 * DSI instances
 * @param DSI_IT: specifies the DSI interrupt sources to be enabled or
 ...
 * @retval None
 */
void DSI_ITConfig(DSI_TypeDef *DSIx, uint32_t DSI_IT,
