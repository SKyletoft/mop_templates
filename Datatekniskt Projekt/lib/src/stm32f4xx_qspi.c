/**
 ...
 RCC_AHB3PeriphClockCmd(RCC_AHB3Periph_QSPI,ENABLE); function.
        ...
        (#) Program the Flash Size, CS High Time, Sample Shift, Prescaler,
 Clock Mode values using the QSPI_Init() function.

        (#) Enable QSPI using  QSPI_Cmd() function.

        (#) Set QSPI Data Length using QSPI_SetDataLength() function.

        (#) Configure the FIFO threshold using QSPI_SetFIFOThreshold() to
 select at which threshold the FTF event is generated.
        ...
        (#) When using the DMA mode
           ...
           (++) Active the needed channel Request using SPI_I2S_DMACmd()
 function.
...
#####
 ===============================================================================
       ...
       [..] < OPTIONAL:
              Add here the most important information to know about the IP
features covered by this group of function.
 ...
 * @param  QSPI_InitStruct: pointer to a QSPI_InitTypeDef structure which will
 * be initialized.
 * @retval None
 */
...
void QSPI_ComConfig_StructInit(
	...
	QSPI_ComConfig_InitStruct->QSPI_ComConfig_SIOOMode =
	    QSPI_ComConfig_SIOOMode_Disable;
	/* Initialize the QSPI_ComConfig_FMode member */
	QSPI_ComConfig_InitStruct->QSPI_ComConfig_FMode =
	...
	QSPI_ComConfig_InitStruct->QSPI_ComConfig_IMode =
	    QSPI_ComConfig_IMode_NoInstruction;
 ...
 *         contains the configuration information for the specified QSPI
 * peripheral.
 * @retval None
 */
 ...
 * @param  QSPI_ComConfig_InitStruct: pointer to a QSPI_ComConfig_InitTypeDef
 * structure that contains the communication configuration informations about
 * QSPI peripheral.
 * @retval None
 */
void QSPI_ComConfig_Init(
    QSPI_ComConfig_InitTypeDef *QSPI_ComConfig_InitStruct) {
	uint32_t tmpreg = 0;

	/* Check the QSPI Communication Control parameters */
	assert_param(
	    IS_QSPI_FMODE(QSPI_ComConfig_InitStruct->QSPI_ComConfig_FMode));
	assert_param(
	    IS_QSPI_SIOOMODE(QSPI_ComConfig_InitStruct->QSPI_ComConfig_SIOOMode));
	assert_param(
	    IS_QSPI_DMODE(QSPI_ComConfig_InitStruct->QSPI_ComConfig_DMode));
	assert_param(
	    IS_QSPI_DCY(QSPI_ComConfig_InitStruct->QSPI_ComConfig_DummyCycles));
	assert_param(
	    IS_QSPI_ABSIZE(QSPI_ComConfig_InitStruct->QSPI_ComConfig_ABSize));
	assert_param(
	    IS_QSPI_ABMODE(QSPI_ComConfig_InitStruct->QSPI_ComConfig_ABMode));
	assert_param(
	    IS_QSPI_ADSIZE(QSPI_ComConfig_InitStruct->QSPI_ComConfig_ADSize));
	assert_param(
	    IS_QSPI_ADMODE(QSPI_ComConfig_InitStruct->QSPI_ComConfig_ADMode));
	assert_param(
	    IS_QSPI_IMODE(QSPI_ComConfig_InitStruct->QSPI_ComConfig_IMode));
	assert_param(
	    IS_QSPI_INSTRUCTION(QSPI_ComConfig_InitStruct->QSPI_ComConfig_Ins));
	assert_param(
	    IS_QSPI_DDRMODE(QSPI_ComConfig_InitStruct->QSPI_ComConfig_DDRMode));
	assert_param(
	    IS_QSPI_DHHC(QSPI_ComConfig_InitStruct->QSPI_ComConfig_DHHC));
	...
	tmpreg |=
	    (uint32_t) ((QSPI_ComConfig_InitStruct->QSPI_ComConfig_FMode)
	                | (QSPI_ComConfig_InitStruct->QSPI_ComConfig_DDRMode)
	                | (QSPI_ComConfig_InitStruct->QSPI_ComConfig_DHHC)
	                | (QSPI_ComConfig_InitStruct->QSPI_ComConfig_SIOOMode)
	                | (QSPI_ComConfig_InitStruct->QSPI_ComConfig_DMode)
	                ...
	                | ((QSPI_ComConfig_InitStruct->QSPI_ComConfig_DummyCycles)
	                   << 18));
...
/**
  ...
  * @param  QSPI_Match: Value to be compared with the masked status register
  to get a match.
  *          This parameter can be any value between 0x00000000 and
  0xFFFFFFFF.
  * @param  QSPI_Mask: Mask to be applied to the status bytes received in
  polling mode..
  *          This parameter can be any value between 0x00000000 and
  0xFFFFFFFF.
  * @param  QSPI_Match_Mode: indicates which method should be used for
  determining a “match” during
  *         automatic polling mode.
  *          This parameter can be any value of :
  *            @arg QSPI_PMM_AND: AND match mode- SMF is set if all the
  unmasked bits received from the flash match
  *                 the corresponding bits in the match register
  *            @arg QSPI_PMM_OR: OR match mode- SMF is set if any one of the
  unmasked bits received from the flash matches its corresponding bit in the
  match register.
  * @note   This function is used only in Automatic Polling Mode
  * @retval None
  */
void QSPI_AutoPollingMode_Config(uint32_t QSPI_Match, uint32_t QSPI_Mask,
...
/**
 * @brief  Sets the number of CLK cycle between two read during automatic
 * polling phases.
 * @param  QSPI_Interval: The number of CLK cycle between two read during
 * automatic polling phases. This parameter can be any value of between 0x0000
 * and 0xFFFF
 * @note   This function is used only in Automatic Polling Mode
 * @retval None
 */
 ...
 * @param  QSPI_Timeout: This field indicates how many CLK cycles QSPI waits
 * after the FIFO becomes full until it raises nCS, putting the flash memory
 ...
 *         This parameter can be any value of between 0x00000000 and
 * 0xFFFFFFFF
 ...
 * @param  QSPI_AlternateByte: Optional data to be send to the external QSPI
 * device right after the address. This parameter can be any value of between
 ...
 *         This parameter can be any value of between 0x00000000 and
 * 0xFFFFFFFF 0x0000_0000: 1 byte is to be transferred 0x0000_0001: 2 bytes
 * are to be transferred 0x0000_0002: 3 bytes are to be transferred
 ...
 *             0xFFFF_FFFF: undefined length -- all bytes until the end of
 * flash memory (as defined by FSIZE) are to be transferred
 ...
 * @param  QSPI_IT: specifies the QSPI interrupt source to be enabled or
 ...
 * @param  None
 * @retval QSPI Functional Mode .The returned value can be one of the
 * following:
