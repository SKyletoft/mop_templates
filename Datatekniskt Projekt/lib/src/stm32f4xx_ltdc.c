/**
 ...
 ===============================================================================
    [..]
        ...
        (#) Configures LTDC
          (++) Configure the required Pixel clock following the panel
 datasheet
        ...
        (#) Configures Layer1/2 parameters
          (++) The Layer window horizontal and vertical position in the
 LTDC_LxWHPCR and LTDC_WVPCR registers. The layer window must be in the active
 data area.
 ...
 register
          ...
          (++) If needed, Dithering and color keying can be enabled
 respectively in the LTDC_GCR and LTDC_LxCKCR registers. It can be also
 enabled on the fly.
        ...
        (#) Reload the shadow registers to active register through
          ...
          -@- All layer parameters can be modified on the fly except the CLUT.
              ...
              or during vertical blanking period by configuring the LTDC_SRCR
 register.
...
/**
 * @brief  Initializes the LTDC peripheral according to the specified
 * parameters in the LTDC_InitStruct.
 * @note   This function can be used only when the LTDC is disabled.
 * @param  LTDC_InitStruct: pointer to a LTDC_InitTypeDef structure that
 * contains the configuration information for the specified LTDC peripheral.
 * @retval None
 */
...
void LTDC_Init(LTDC_InitTypeDef *LTDC_InitStruct) {
	...
	LTDC->GCR |= (uint32_t) (LTDC_InitStruct->LTDC_HSPolarity
	...
	LTDC->BCCR |=
	    (backred | backgreen | LTDC_InitStruct->LTDC_BackgroundBlueValue);
}
...
void LTDC_StructInit(LTDC_InitTypeDef *LTDC_InitStruct) {
	...
	LTDC_InitStruct->LTDC_VerticalSync =
	    0x00; /*!< Initialize the LTDC_VerticalSync member */
	...
	LTDC_InitStruct->LTDC_TotalWidth =
	    0x00; /*!< Initialize the LTDC_TotalWidth member */
	LTDC_InitStruct->LTDC_TotalHeigh =
	    0x00; /*!< Initialize the LTDC_TotalHeigh member */
 ...
 * @param  LTDC_RGB_InitStruct: pointer to a LTDC_RGBTypeDef structure that
 * contains the Dither RGB width.
 * @retval None
 */
 ...
 * @param  LTDC_RGB_InitStruct: pointer to a LTDC_RGBTypeDef structure which
 * will be initialized.
 * @retval None
 */
 ...
 * @note   This function can be used only when the LTDC is disabled.
 * @param  LTDC_layerx: Select the layer to be configured, this parameter can
 * be one of the following values: LTDC_Layer1, LTDC_Layer2
 * @param  LTDC_LayerStruct: pointer to a LTDC_LayerTypeDef structure that
 * contains the configuration information for the specified LTDC peripheral.
 * @retval None
 */
...
void LTDC_LayerInit(LTDC_Layer_TypeDef *LTDC_Layerx,
	...
	/* Check the parameters */
	assert_param(
	    IS_LTDC_Pixelformat(LTDC_Layer_InitStruct->LTDC_PixelFormat));
	assert_param(IS_LTDC_BlendingFactor1(
	    LTDC_Layer_InitStruct->LTDC_BlendingFactor_1));
	assert_param(IS_LTDC_BlendingFactor2(
	    LTDC_Layer_InitStruct->LTDC_BlendingFactor_2));
	assert_param(
	    IS_LTDC_HCONFIGST(LTDC_Layer_InitStruct->LTDC_HorizontalStart));
	assert_param(
	    IS_LTDC_HCONFIGSP(LTDC_Layer_InitStruct->LTDC_HorizontalStop));
	assert_param(
	    IS_LTDC_VCONFIGST(LTDC_Layer_InitStruct->LTDC_VerticalStart));
	...
	LTDC_Layerx->WVPCR =
	    (LTDC_Layer_InitStruct->LTDC_VerticalStart | wvsppos);
	...
	LTDC_Layerx->DCCR = (LTDC_Layer_InitStruct->LTDC_DefaultColorBlue
	                     | dcgreen | dcred | dcalpha);
 ...
 * @param  LTDC_Layer_InitStruct: pointer to a LTDC_LayerTypeDef structure
 ...
 * @param  LTDC_Pos_InitStruct: pointer to a LTDC_PosTypeDef structure that
 ...
 * @param  LTDC_Pos_InitStruct: pointer to a LTDC_PosTypeDef structure which
 * will be initialized.
 * @retval None
 */
 ...
 * @param  LTDC_colorkeying_InitStruct: pointer to a
 * LTDC_ColorKeying_InitTypeDef structure that contains the color keying
 * configuration.
 * @param  LTDC_layerx: Select the layer to be configured, this parameter can
 * be one of the following values: LTDC_Layer1, LTDC_Layer2
 * @retval None
...
/**
 * @brief  Fills each LTDC_colorkeying_InitStruct member with its default
 * value.
 * @param  LTDC_colorkeying_InitStruct: pointer to a
 * LTDC_ColorKeying_InitTypeDef structure which will be initialized.
 * @retval None
 */
 ...
 * @param  LTDC_layerx: Select the layer to be configured, this parameter can
 * be one of the following values: LTDC_Layer1, LTDC_Layer2 This parameter can
 ...
 * @param  LTDC_CLUT_InitStruct: pointer to a LTDC_CLUT_InitTypeDef structure
 * that contains the CLUT configuration.
 * @param  LTDC_layerx: Select the layer to be configured, this parameter can
 * be one of the following values: LTDC_Layer1, LTDC_Layer2
 * @retval None
 */
...
void LTDC_CLUTInit(LTDC_Layer_TypeDef *LTDC_Layerx,
	...
	LTDC_Layerx->CLUTWR =
	    (clutadd | LTDC_CLUT_InitStruct->LTDC_BlueValue | green | red);
}
...
void LTDC_LayerPosition(LTDC_Layer_TypeDef *LTDC_Layerx, uint16_t OffsetX,
	...
	horizontal_stop =
	    (((tempreg & 0x1FFF) - 3) / temp) + horizontal_start - 1;
 ...
 * @param  LTDC_layerx: Select the layer to be configured, this parameter can
 * be one of the following values: LTDC_Layer1, LTDC_Layer2
 * @retval Reload of the shadow registers values must be applied after
 * constant alpha reconfiguration.
 */
...
@verbatim
 ...
 ===============================================================================
    ...
    [..] This section provides functions allowing to configure the LTDC
Interrupts and to get the status and clear flags and Interrupts pending bits.
    ...
    *** Interrupts ***
    ...
    [..]
      ...
      (+) LTDC_IT_FU: FIFO Underrun interrupt is generated when a pixel is
requested from an empty layer FIFO
      ...
      (+) LTDC_IT_RR: Register Reload interrupt is generated when the shadow
                      registers reload was performed during the vertical
blanking period.
 ...
 * @param  LTDC_IT: specifies the LTDC interrupts sources to be enabled or
