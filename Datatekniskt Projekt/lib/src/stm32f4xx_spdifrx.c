...
@verbatim
 ...
 ===============================================================================
  [..]
  This section provides a set of functions allowing to initialize the SPDIFRX
Audio

  Block Mode, Audio Protocol, Data size, Synchronization between audio block,
  Master clock Divider, FIFO threshold, Frame configuration, slot
configuration, Tristate mode, Companding mode and Mute mode.
  [..]
  The SPDIFRX_Init(), SPDIFRX_FrameInit() and SPDIFRX_SlotInit() functions
follows the SPDIFRX Block configuration procedures for Master mode and Slave
mode (details for these procedures are available in reference manual(RMxxxx).
...
/**
 * @brief  Deinitialize the SPDIFRXx peripheral registers to their default
 * reset values.
 * @param  void
 * @retval None
 */
 ...
 *
 * @note   SPDIFRX clock is generated from a specific output of the PLLSPDIFRX
 * or a specific output of the PLLI2S or from an alternate function bypassing
 * the PLL I2S.
 *
 * @param  SPDIFRX_InitStruct: pointer to a SPDIFRX_InitTypeDef structure that
 *         contains the configuration information for the specified SPDIFRX
 * Block peripheral.
 * @retval None
 */
void SPDIFRX_Init(SPDIFRX_InitTypeDef *SPDIFRX_InitStruct) {
	...
	assert_param(
	    IS_SPDIFRX_INPUT_SELECT(SPDIFRX_InitStruct->SPDIFRX_InputSelection));
	assert_param(IS_SPDIFRX_MAX_RETRIES(SPDIFRX_InitStruct->SPDIFRX_Retries));
	assert_param(IS_SPDIFRX_WAIT_FOR_ACTIVITY(
	    SPDIFRX_InitStruct->SPDIFRX_WaitForActivity));
	assert_param(
	    IS_SPDIFRX_CHANNEL(SPDIFRX_InitStruct->SPDIFRX_ChannelSelection));
	assert_param(
	    IS_SPDIFRX_DATA_FORMAT(SPDIFRX_InitStruct->SPDIFRX_DataFormat));
	...
	/* Configure SPDIFRX: Input selection, Maximum allowed re-tries during
	synchronization phase,
	wait for activity, Channel Selection, Data samples format and stereo/mono
	mode */
 ...
 * @param  SPDIFRX_InitStruct: pointer to a SPDIFRX_InitTypeDef structure
 ...
 * @param  SPDIFRX_IT: specifies the SPDIFRX interrupt source to be enabled or
 ...
 *            @arg SPDIFRX_IT_SBLKIE: Synchronization Block Detected Interrupt
 * Enable
 ...
 *            @arg SPDIFRX_FLAG_CSRNE: The Control Buffer register is not
 * empty flag.
...
ITStatus SPDIFRX_GetITStatus(uint32_t SPDIFRX_IT) {
	...
	if (((SPDIFRX->SR & SPDIFRX_IT) != (uint32_t) RESET)
 ...
 *            @arg SPDIFRX_IT_AFSDET: anticipated frame synchronization
 * detection interrupt.
 *            @arg SPDIFRX_IT_LFSDET: late frame synchronization detection
 * interrupt.
 *
 * @note    FREQ (FIFO Request) flag is cleared :
 *          - When the audio block is transmitter and the FIFO is full or the
 * FIFO has one data (one buffer mode) depending the bit FTH in the
