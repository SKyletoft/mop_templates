 ...
 *** HASH operation : ***
 ...
 [..]
   ...
   (#) Reset the HASH processor core, so that the HASH will be ready
       to compute he message digest of a new message by using HASH_Reset()
function.
   ...
   (#) if the message length is not an exact multiple of 512 bits,
       then the function HASH_StartDigest() must be called to launch the
computation of the final digest.
...
HASH_ClearFlag() function. And after checking on an interrupt event you should
 ...
 *** HMAC operation : ***
 ...
 [..] Basically, the HMAC algorithm consists of two nested hash operations:
      HMAC(message) = Hash[((key | pad) XOR 0x5C) | Hash(((key | pad) XOR
0x36) | message)] where:
 ...
 [..]To compute the HMAC, four different phases are required:
   ...
   (#) The key (to be used for the inner hash function) is then given to the
core. This operation follows the same mechanism as the one used to send the
   ...
   (#) Once the last word has been entered and computation has started,
       the hash processor elaborates the key. It is then ready to accept the
message text using the same mechanism as the one used to send the message in
the hash operation.

   (#) After the first hash round, the hash processor returns "ready" to
indicate that it is ready to receive the key to be used for the outer hash
function (normally, this key is the same as the one used for the inner hash
function). When the last word of the key is entered and computation starts,
the HMAC result is made available using HASH_GetDigest() function.
...
/**
 * @brief  De-initializes the HASH peripheral registers to their default reset
 * values
 * @param  None
 * @retval None
 */
...
/**
 * @brief  Initializes the HASH peripheral according to the specified
 ...
 * @param  HASH_InitStruct: pointer to a HASH_InitTypeDef structure that
 * contains the configuration information for the HASH peripheral.
...
void HASH_Init(HASH_InitTypeDef *HASH_InitStruct) {
	...
	HASH->CR |=
	    (HASH_InitStruct->HASH_AlgoSelection | HASH_InitStruct->HASH_DataType
	...
	/* Reset the HASH processor core, so that the HASH will be ready to
 ...
 * @param  HASH_InitStruct : pointer to a HASH_InitTypeDef structure which
 * will be initialized.
 *  @note  The default values set are : Processor mode is HASH, Algorithm
 * selected is SHA1, Data type selected is 32b and HMAC Key Type is short key.
 * @retval None
 */
 ...
 * @note   Calling this function will clear the HASH_SR_DCIS (Digest
 * calculation completion interrupt status) bit corresponding to HASH_IT_DCI
...
@verbatim
 ===============================================================================
                  ##### Message Digest generation functions #####
 ===============================================================================
 [..] This section provides functions allowing the generation of message
digest:
   (+) Push data in the IN FIFO : using HASH_DataIn()
   (+) Get the number of words set in IN FIFO, use HASH_GetInFIFOWordsNbr()
   (+) set the last word valid bits number using
 ...
 * @note   In MD5 mode, Data[7] to Data[4] filed of HASH_MsgDigest structure
 * is not used and is read as zero. In SHA-1 mode, Data[7] to Data[5] filed of
 * HASH_MsgDigest structure is not used and is read as zero. In SHA-224 mode,
 * Data[7] filed of HASH_MsgDigest structure is not used and is read as zero.
 * @param  HASH_MessageDigest: pointer to a HASH_MsgDigest structure which
 * will hold the message digest result
 * @retval None
 */
...
@verbatim
 ...
 [..] It is possible to interrupt a HASH/HMAC process to perform another
processing with a higher priority, and to complete the interrupted process
later on, when the higher priority task is complete. To do so, the context of
the interrupted task must be saved from the HASH registers to memory, and then
be restored from memory to the HASH registers.
 ...
 * @note   After calling this function, user can restart the processing from
 * the point where it has been interrupted.
 * @param  HASH_ContextRestore: pointer to a HASH_Context structure that
...
for HASH/ HMAC data input transfer.

 [..] When the DMA mode is enabled (using the HASH_DMACmd() function), data
can be sent to the IN FIFO using the DMA peripheral.
...
void HASH_AutoStartDigest(FunctionalState NewState) {
	...
	if (NewState != DISABLE) {
		/* Enable the auto start of the final message digest at the end of DMA
		 * transfer */
		HASH->CR &= ~HASH_CR_MDMAT;
	} else {
		/* Disable the auto start of the final message digest at the end of
		 * DMA transfer */
		HASH->CR |= HASH_CR_MDMAT;
	}
}
...
@verbatim
 ...
 [..] This section provides functions allowing to configure the HASH
Interrupts and to get the status and clear flags and Interrupts pending bits.
 ...
 *** Flags : ***
 ...
 [..]
   (#) HASH_FLAG_DINIS : set when 16 locations are free in the Data IN FIFO
      which means that a  new block (512 bit) can be entered into the input
buffer.
   ...
   (#) HASH_FLAG_DINNE : set when Data IN FIFO is not empty which means that
       the Data IN FIFO contains at least one word of data. This Flag is
cleared only by hardware.

 *** Interrupts : ***
 ====================
 [..]
   (#) HASH_IT_DINI  : if enabled, this interrupt source is pending when 16
       locations are free in the Data IN FIFO  which means that a new block
(512 bit) can be entered into the input buffer. This interrupt source is
cleared using HASH_ClearITPendingBit(HASH_IT_DINI) function.

   (#) HASH_IT_DCI   : if enabled, this interrupt source is pending when
Digest calculation is complete. This interrupt source is cleared using
       HASH_ClearITPendingBit(HASH_IT_DCI) function.

 *** Managing the HASH controller events : ***
 =============================================
 [..] The user should identify which mode will be used in his application to
manage the HASH controller events: Polling mode or Interrupt mode.
 ...
 * @param  HASH_IT: specifies the HASH interrupt source to be enabled or
 ...
 *            @arg HASH_FLAG_DCIS: Digest calculation completion interrupt
 * status flag
 *            @arg HASH_FLAG_BUSY: Busy flag
 *            @arg HASH_FLAG_DMAS: DMAS Status flag
 *            @arg HASH_FLAG_DINNE: Data Input register (DIN) not empty status
 * flag
