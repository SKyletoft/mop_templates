...
data (using polling/interrupt mode). For more details, refer to "Interrupts
and flags management functions" module description.
 ...
 */
    ...
    || defined(STM32F410xx) || defined(STM32F412xG)                          \
    || defined(STM32F413_423xx) || defined(STM32F429_439xx)                  \
...
@verbatim
 ...
 ===============================================================================
 [..] This section provides a function allowing to get the 32 bit Random
number
 ...
 *
 * @note   Before to call this function you have to wait till DRDY (data
 * ready) flag is set, using RNG_GetFlagStatus(RNG_FLAG_DRDY) function.
 * @note   Each time the Random number data is read (using
 * RNG_GetRandomNumber() function), the RNG_FLAG_DRDY flag is automatically
 * cleared.
 * @note   In the case of a seed error, the generation of random numbers is
 *         interrupted for as long as the SECS bit is '1'. If a number is
 *         available in the RNG_DR register, it must not be used because it
 * may not have enough entropy. In this case, it is recommended to clear the
 ...
 * provide the RNG clock and clear the CEIS bit (using
 * RNG_ClearFlag(RNG_FLAG_CECS) function) . The clock error has no impact on
 * the previously generated random numbers, and the RNG_DR register contents
 * can be used.
 *
 * @param  None
...
RNG_ClearITPendingBit(RNG_IT_SEI) function; or
   (#) In the case of a clock error : the PLL48CLK (RNG peripheral clock
source) was not correctly detected (fPLL48CLK< fHCLK/16). This interrupt
source is cleared using RNG_ClearITPendingBit(RNG_IT_CEI) function.
...
manage the RNG controller events: Polling mode or Interrupt mode.
...
only by reading the Random number data.
 ...
 *         (see RNG_GetITStatus() function) and clear bit except the DRDY
 * event (see RNG_ClearITPendingBit() function).
