/**
  ...
  *          It uses the stm32f4xx_cryp.c/.h drivers to access the STM32F4xx
CRYP
  *          peripheral.
  *          AES-ECB/CBC/CTR/GCM/CCM modes are available on STM32F437x
Devices.
...
@verbatim
 ...
 [..]
   ...
   (#) Encrypt and decrypt using AES in ECB Mode using CRYP_AES_ECB()
function.

   (#) Encrypt and decrypt using AES in CBC Mode using CRYP_AES_CBC()
function.

   (#) Encrypt and decrypt using AES in CTR Mode using CRYP_AES_CTR()
function.

   (#) Encrypt and decrypt using AES in GCM Mode using CRYP_AES_GCM()
function.

   (#) Encrypt and decrypt using AES in CCM Mode using CRYP_AES_CCM()
function.
...
ErrorStatus CRYP_AES_ECB(uint8_t Mode, uint8_t *Key, uint16_t Keysize,
	...
	case 128:
		...
		AES_CRYP_KeyInitStructure.CRYP_Key2Left =
		    __REV(*(uint32_t *) (keyaddr));
		keyaddr += 4;
		AES_CRYP_KeyInitStructure.CRYP_Key2Right =
		    __REV(*(uint32_t *) (keyaddr));
		keyaddr += 4;
		AES_CRYP_KeyInitStructure.CRYP_Key3Left =
		    __REV(*(uint32_t *) (keyaddr));
		keyaddr += 4;
		AES_CRYP_KeyInitStructure.CRYP_Key3Right =
		    __REV(*(uint32_t *) (keyaddr));
		break;
	case 192:
		AES_CRYP_InitStructure.CRYP_KeySize = CRYP_KeySize_192b;
		AES_CRYP_KeyInitStructure.CRYP_Key1Left =
		    __REV(*(uint32_t *) (keyaddr));
		keyaddr += 4;
		AES_CRYP_KeyInitStructure.CRYP_Key1Right =
		    __REV(*(uint32_t *) (keyaddr));
		keyaddr += 4;
		AES_CRYP_KeyInitStructure.CRYP_Key2Left =
		    __REV(*(uint32_t *) (keyaddr));
	...
	case 256:
		...
		AES_CRYP_KeyInitStructure.CRYP_Key0Left =
		    __REV(*(uint32_t *) (keyaddr));
		keyaddr += 4;
		AES_CRYP_KeyInitStructure.CRYP_Key0Right =
		    __REV(*(uint32_t *) (keyaddr));
		keyaddr += 4;
		AES_CRYP_KeyInitStructure.CRYP_Key1Left =
		    __REV(*(uint32_t *) (keyaddr));
 ...
 * @param  Keysize: length of the Key, must be a 128, 192 or 256.
 * @param  Input: pointer to the Input buffer.
 * @param  Ilength: length of the Input buffer in bytes, must be a multiple
 * of 16.
 * @param  Header: pointer to the header buffer.
 * @param  Hlength: length of the header buffer in bytes, must be a multiple
 * of 16.
 * @param  Output: pointer to the returned buffer.
...
ErrorStatus CRYP_AES_GCM(uint8_t Mode, uint8_t InitVectors[16], uint8_t *Key,
	...
	if (Mode == MODE_ENCRYPT) /* AES encryption */
	{
		...
		if (HLength != 0) {
			...
			if (CRYP_GetCmdStatus() == DISABLE) {
				/* The CRYP peripheral clock is not enabled or the device
				   doesn't embed
				   the CRYP peripheral (please check the device sales type. */
				return (ERROR);
			}

			for (loopcounter = 0; (loopcounter < HLength);
			     loopcounter += 16) {
		...
		if (ILength != 0) {
			...
			for (loopcounter = 0;
			     ((loopcounter < ILength) && (status != ERROR));
			     loopcounter += 16) {
				/* Wait until the IFEM flag is reset */
				while (CRYP_GetFlagStatus(CRYP_FLAG_IFEM) == RESET) {}
				/* Write the Input block in the IN FIFO */
 ...
 *            @arg MODE_ENCRYPT: Encryption
 *            @arg MODE_DECRYPT: Decryption
 * @param  Nonce: the nonce used for AES algorithm. It shall be unique for
 * each processing.
 * @param  Key: Key used for AES algorithm.
 * @param  Keysize: length of the Key, must be a 128, 192 or 256.
 * @param  Input: pointer to the Input buffer.
...
ErrorStatus CRYP_AES_CCM(uint8_t Mode, uint8_t *Nonce, uint32_t NonceSize,
	 ...
	 * ***********************/
	if (headersize != 0) {
		/* Check that the associated data (or header) length is lower than
		...
		if (headersize < 65280) {
			...
			/* header is encoded as 0xff || 0xfe || [headersize]32, i.e., six
			 * octets */
		...
		if ((headersize % 16) != 0) {
			/* Padd the header buffer with 0s till the HBuffer length is
			 * modulo 16 */
			for (loopcounter = headersize;
			     loopcounter <= ((headersize / 16) + 1) * 16; loopcounter++) {
	...
	/* Byte 0:
	   ...
	   blocks are distinct from B0 Bits 0, 1, and 2 contain the same encoding
	   of q as in B0
	*/
	...
	if (Mode == MODE_ENCRYPT) /* AES encryption */
	{
		...
		if (headersize != 0) {
			...
			for (loopcounter = 0; (loopcounter < headersize);
			     loopcounter += 16) {
				/* Wait until the IFEM flag is reset */
				while (CRYP_GetFlagStatus(CRYP_FLAG_IFEM) == RESET) {}
		...
		CRYP_DataIn(*(uint32_t *) (ctraddr));
		ctraddr += 4;
		/* Reset bit 0 (after 8-bit swap) is equivalent to reset bit 24
		 * (before 8-bit swap) */
	...
	for (loopcounter = 0; (loopcounter < TAGSize); loopcounter++) {
		...
		*((uint8_t *) tagaddr + loopcounter) =
		    *((uint8_t *) temptag + loopcounter);
	}
