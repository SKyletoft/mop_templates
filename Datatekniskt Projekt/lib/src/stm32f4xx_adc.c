/**
 ...
 peripheral:
  *           + Initialization and Configuration (in addition to ADC multi
 mode
  *             selection)
 ...
 ADC_DMARequestAfterLastTransferCmd() function.
     ...
     *** Injected channels group configuration ***
     ...
     [..]
       ...
       (+) To activate the AutoInjected mode, use the
 ...
 * configuration parameters are specific to the regular channels group (scan
 * mode activation, continuous mode activation, External trigger source and
 *         edge, number of conversion in the regular channels group
 * sequencer).
...
void ADC_CommonInit(ADC_CommonInitTypeDef *ADC_CommonInitStruct) {
	...
	/* Configure ADCx: Multi mode, Delay between two sampling time, ADC
	   prescaler, and DMA access mode for multimode */
...
Watchdog (AWD) feature in the ADC.
...
internal connections between the ADC and the Temperature Sensor, the Vrefint
and the Vbat sources.

    [..] A typical configuration to get the Temperature sensor and Vrefint
channels voltages is done following these steps :
      (#) Enable the internal connection of Temperature sensor and Vrefint
sources with the ADC channels using ADC_TempSensorVrefintCmd() function.
 ...
 *         STM32F41xx devices and equal to VBAT/4 on STM32F42xx and STM32F43xx
 * devices
 *
...
subsection provides functions allowing to configure the ADC regular channels :
         (++) Configure the rank in the regular group sequencer for each
channel
...
function in the ADC peripheral since it returns the converted data of the
current regular channel. When the Conversion value is read, the EOC Flag is
          automatically cleared.

          -@- For multi ADC mode, the last ADC1, ADC2 and ADC3 regular
conversions results data (in the selected multi mode) can be returned in the
same time using ADC_GetMultiModeConversionValue() function.
...
void ADC_EOCOnEachRegularChannelCmd(ADC_TypeDef *ADCx,
	...
	if (NewState != DISABLE) {
		/* Enable the selected ADC EOC rising on each regular channel
		 * conversion */
		ADCx->CR2 |= (uint32_t) ADC_CR2_EOCS;
	} else {
		/* Disable the selected ADC EOC rising on each regular channel
		 * conversion */
		ADCx->CR2 &= (uint32_t) (~ADC_CR2_EOCS);
	}
}
 ...
 * value. This number must be between 1 and 8.
 * @retval None
 */
 ...
 * ADC1, ADC3 and ADC2. Data[31:16]: these bits contain alternatively the
 * regular data of ADC2, ADC1 and ADC3.
 */
...
regular channel. This avoids the loss of the data already stored in the ADC
         Data register.
...
function), at the end of the last DMA transfer, two possibilities are allowed:
...
configuration (using the void ADC_MultiModeDMARequestAfterLastTransferCmd()
function), at the end of the last DMA transfer, two possibilities are allowed:
        (+) No new DMA request is issued to the DMA controller (feature
DISABLED)
        (+) Requests can continue to be generated (feature ENABLED).
 ...
 * transfer. This parameter can be: ENABLE or DISABLE.
 * @retval None
 */
...
/**
 * @brief  Enables or disables the ADC DMA request after last transfer in
 * multi ADC mode
 * @param  NewState: new state of the selected ADC DMA request after last
 * transfer. This parameter can be: ENABLE or DISABLE.
 * @note   if Enabled, DMA requests are issued as long as data are converted
 * and DMA mode for multi ADC mode (selected using ADC_CommonInit() function
...
provides functions allowing to configure the ADC injected channels :
        (++) Configure the rank in the injected group sequencer for each
channel
        (++) Configure the sampling time for each channel
      ...
      (#) Get the Specified Injected channel conversion data: This subsection
          provides an important function in the ADC peripheral since it
returns the converted data of the specific injected channel.
 ...
 * edge to start injected conversion. This parameter can be one of the
 * following values:
 ...
 * @param  ADCx: where x can be 1, 2 or 3 to select the ADC peripheral.
 * @param  NewState: new state of the selected ADC discontinuous mode on
 * injected group channel. This parameter can be: ENABLE or DISABLE.
 * @retval None
 */
...
Interrupts and to get the status and clear flags and Interrupts pending bits.
    ...
    [..] Each ADC provides 4 Interrupts sources and 6 Flags which can be
...
lost
        ...
        (##) ADC_FLAG_EOC : Regular channel end of conversion ==> to indicate
             (depending on EOCS bit, managed by
ADC_EOCOnEachRegularChannelCmd() ) the end of:
  ...
  *** Flags and Interrupts for ADC Injected channels ***
  ======================================================
    [..]
      (+) Flags :
        (##) ADC_FLAG_JEOC : Injected channel end of conversion ==> to
indicate at the end of injected GROUP conversion

        (##) ADC_FLAG_JSTRT: Injected channel start ==> to indicate hardware
when injected GROUP conversion starts.
    [..]
      (+) Interrupts :
...
to manage the ADC controller events: Polling mode or Interrupt mode.
