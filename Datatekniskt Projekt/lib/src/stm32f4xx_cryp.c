/**
  ...
  *          functionalities of the  Cryptographic processor (CRYP)
peripheral:
...
@verbatim
 ...
 [..]
   ...
   (#) If using DMA for Data input and output transfer, activate the needed
DMA Requests using CRYP_DMACmd() function

   (#) If DMA is not used for data transfer, use CRYP_DataIn() and
CRYP_DataOut() functions to enter data to IN FIFO and get result from OUT
FIFO.
...
interrupt routines in processing phase.
   ...
   (#) Save and restore Cryptographic processor context using
 ...
 *** Initialization ***
 ======================
 [..]
   (#) Initialize the peripheral using CRYP_Init(), CRYP_KeyInit() and
 ...
 *** Interrupts Processing mode ***
 ...
 [..] In this mode, Processing is done when the data are transferred by the
    ...
    (#) Enable the interrupts CRYP_IT_INI and CRYP_IT_OUTI using
CRYP_ITConfig() function.
    ...
    (#) In the CRYP_IT_INI interrupt handler : load the input message into the
         ...
         the message has been entered into the IN FIFO, disable the
CRYP_IT_INI interrupt (using CRYP_ITConfig() function).

    (#) In the CRYP_IT_OUTI interrupt handler : read the output message from
         the OUT FIFO using CRYP_DataOut() function. You can read 1 block (2
or 4 words) at a time or read data until the FIFO is empty. When the last word
has been read, INIM=0, BUSY=0 and both FIFOs are empty (CRYP_FLAG_IFEM is set
and CRYP_FLAG_OFNE is reset). You can disable the CRYP_IT_OUTI interrupt
(using CRYP_ITConfig() function) and you can disable the peripheral using
CRYP_Cmd() function.
 ...
 [..] In this mode, Processing is done when the DMA is used to transfer the
    ...
    (#) Configure the DMA controller to transfer the input data from the
     ...
     (#) All the transfers and processing are managed by the DMA and the
         cryptographic processor. The DMA transfer complete interrupt
indicates that the processing is complete. Both FIFOs are normally empty and
...
@endverbatim
  ...
  */
/**
 * @brief  Deinitializes the CRYP peripheral registers to their default reset
 * values
 * @param  None
 * @retval None
 */
...
/**
 * @brief  Initializes the CRYP peripheral according to the specified
 * parameters in the CRYP_InitStruct.
 * @param  CRYP_InitStruct: pointer to a CRYP_InitTypeDef structure that
 * contains the configuration information for the CRYP peripheral.
 * @retval None
 */
void CRYP_Init(CRYP_InitTypeDef *CRYP_InitStruct) {
	...
	if ((CRYP_InitStruct->CRYP_AlgoMode != CRYP_AlgoMode_TDES_ECB)
		...
		CRYP->CR |= CRYP_InitStruct->CRYP_KeySize; /* Key size and value must
		                                              be configured once the
 ...
 * contains the configuration information for the CRYP Initialization
 * Vectors(IV).
 * @retval None
 */
 ...
 * @param  CRYP_Phase: specifies the CRYP AES-CCM and AES-GCM phase to be
...
/**
 * @brief  Flushes the IN and OUT FIFOs (that is read and write pointers of
 * the FIFOs are reset)
...
@verbatim
 ...
 [..] It is possible to interrupt an encryption/ decryption/ key generation
process to perform another processing with a higher priority, and to complete
the interrupted process later on, when the higher-priority task is complete.
To do so, the context of the interrupted task must be saved from the CRYP
registers to memory, and then be restored from memory to the CRYP registers.
...
ErrorStatus CRYP_SaveContext(CRYP_Context *CRYP_ContextSave,
	...
	/* Stop DMA transfers on the IN FIFO by clearing the DIEN bit in the
	 * CRYP_DMACR */
	...
	if ((CRYP->CR
	     & (uint32_t) (CRYP_CR_ALGOMODE_TDES_ECB | CRYP_CR_ALGOMODE_TDES_CBC))
	    != (uint32_t) 0) /* TDES */
	{
	...
	if ((CRYP->SR & ckeckmask) != CRYP_SR_IFEM) {
		...
		/* Save the current configuration (bit 19, bit[17:16] and bits [9:2]
 ...
 *         after restoring the context, you have to enable the DMA again (if
 * the DMA was previously used).
 * @param  CRYP_ContextRestore: pointer to a CRYP_Context structure that
 * contains the repository for saved context.
 * @note   The data that were saved during context saving must be rewritten
 * into the IN FIFO.
 * @retval None
 */
...
@verbatim
 ...
 ===============================================================================
 [..] This section provides functions allowing to configure the DMA interface
for CRYP data input and output transfer.

 [..] When the DMA mode is enabled (using the CRYP_DMACmd() function), data
can be transferred:
   (+) From memory to the CRYP IN FIFO using the DMA peripheral by enabling
       the CRYP_DMAReq_DataIN request.
   (+) From the CRYP OUT FIFO to the memory using the DMA peripheral by
enabling the CRYP_DMAReq_DataOUT request.
 ...
 * @param  CRYP_DMAReq: specifies the CRYP DMA transfer request to be enabled
...
@verbatim
 ...
 [..] This section provides functions allowing to configure the CRYP
Interrupts and to get the status and Interrupts pending bits.
 ...
 *** Flags : ***
 ===============
 [..]
   (#) CRYP_FLAG_IFEM :  Set when Input FIFO is empty. This Flag is cleared
only by hardware.
   ...
   (#) CRYP_FLAG_INRIS  : Set when Input FIFO Raw interrupt is pending it
gives the raw interrupt state prior to masking of the input FIFO service
interrupt. This Flag is cleared only by hardware.

   (#) CRYP_FLAG_OFNE   : Set when Output FIFO not empty. This Flag is cleared
       only by hardware.

   (#) CRYP_FLAG_OFFU   : Set when Output FIFO is full. This Flag is cleared
only by hardware.

   (#) CRYP_FLAG_OUTRIS : Set when Output FIFO Raw interrupt is pending it
gives the raw interrupt state prior to masking of the output FIFO service
interrupt. This Flag is cleared only by hardware.

   (#) CRYP_FLAG_BUSY   : Set when the CRYP core is currently processing a
block of data or a key preparation (for AES decryption). This Flag is cleared
       only by hardware. To clear it, the CRYP core must be disabled and the
 ...
 *** Interrupts : ***
 ...
 [..]
   (#) CRYP_IT_INI   : The input FIFO service interrupt is asserted when there
      ...
      -@- This interrupt is cleared by performing write operations to the
input FIFO until it holds 4 or more words. The input FIFO service interrupt
INMIS is enabled with the CRYP enable bit. Consequently, when CRYP is
disabled, the INMIS signal is low even if the input FIFO is empty.



   (#) CRYP_IT_OUTI  : The output FIFO service interrupt is asserted when
there is one or more (32-bit word) data items in the output FIFO. This
interrupt is associated to CRYP_FLAG_OUTRIS flag.

       -@- This interrupt is cleared by reading data from the output FIFO
until there is no valid (32-bit) word left (that is, the interrupt follows the
state of the OFNE (output FIFO not empty) flag).

 *** Managing the CRYP controller events : ***
 =============================================
 [..] The user should identify which mode will be used in his application to
manage the CRYP controller events: Polling mode or Interrupt mode.

   (#) In the Polling Mode it is advised to use the following functions:
       (++) CRYP_GetFlagStatus() : to check if flags events occur.

       -@@- The CRYPT flags do not need to be cleared since they are cleared
as soon as the associated event are reset.
   ...
   (#) In the Interrupt Mode it is advised to use the following functions:
       ...
       -@@- The CRYPT interrupts have no pending bits, the interrupt is
cleared as soon as the associated event is reset.
 ...
 * @param  CRYP_IT: specifies the CRYP interrupt source to be enabled or
