...
#define CR_HSION_BB                                                          \
	(PERIPH_BB_BASE + (CR_OFFSET * 32) + (HSION_BitNumber * 4))
/* Alias word address of CSSON bit */
#define CSSON_BitNumber 0x13
#define CR_CSSON_BB                                                          \
	(PERIPH_BB_BASE + (CR_OFFSET * 32) + (CSSON_BitNumber * 4))
/* Alias word address of PLLON bit */
#define PLLON_BitNumber 0x18
#define CR_PLLON_BB                                                          \
	(PERIPH_BB_BASE + (CR_OFFSET * 32) + (PLLON_BitNumber * 4))
...
#define CR_PLLSAION_BB                                                       \
	(PERIPH_BB_BASE + (CR_OFFSET * 32) + (PLLSAION_BitNumber * 4))
...
#####
 ===================================================================================
    [..]
      This section provide functions allowing to configure the
internal/external clocks, PLLs, CSS and MCO pins.
...
FS (48 MHz), the random analog generator (<=48 MHz) and the SDIO (<= 48 MHz).
      ...
      (#) PLLI2S (clocked by HSI or HSE), used to generate an accurate clock
to achieve high-quality audio performance on the I2S interface or SAI
interface in case of STM32F429x/439x devices.

      (#) PLLSAI clocked by (HSI or HSE), used to generate an accurate clock
to SAI interface and LCD TFT controller available only for
...
or PLLI2S clock (through a configurable prescaler) on PC9 pin.
 @endverbatim
  * @{
  */
...
void RCC_DeInit(void) {
	...
	/* Reset HSEON, CSSON, PLLON, PLLI2S and
	 * PLLSAI(STM32F42xxx/43xxx/446xx/469xx/479xx devices) bits */
	...
	RCC->PLLI2SCFGR = 0x20003000;
          ...
          STM32F401xx || STM32F411xE || STM32F446xx || STM32F413_423xx ||    \
          STM32F469_479xx */

#if defined(STM32F40_41xxx) || defined(STM32F427_437xx)                      \
    || defined(STM32F429_439xx) || defined(STM32F446xx)                      \
    || defined(STM32F469_479xx)
	/* Reset PLLSAICFGR register, only available for
	 * STM32F42xxx/43xxx/446xx/469xx/479xx devices */
	...
	/* Disable Timers clock prescalers selection, only available for
	 * STM32F42/43xxx and STM32F413_423xx devices */
	RCC->DCKCFGR = 0x00000000;

#if defined(STM32F410xx) || defined(STM32F413_423xx)
	/* Disable LPTIM and FMPI2C clock prescalers selection, only available for
	 * STM32F410xx and STM32F413_423xx devices */
 ...
 * application software should wait on HSERDY flag to be set indicating that
 * HSE clock is stable and can be used to clock the PLL and/or system clock.
 * @note   HSE state can not be changed if it is used directly or through the
 *         PLL as system clock. In this case, you have to select another
 * source of the system clock then change the HSE state (ex. disable it).
 * @note   The HSE is stopped by hardware when entering STOP and STANDBY
 * modes.
 * @note   This function reset the CSSON bit, so if the Clock security
 * system(CSS) was previously enabled you have to enable it again after
 ...
 * constant HSE_STARTUP_TIMEOUT in stm32f4xx.h file. You can tailor it
 * depending on the HSE crystal used in your application.
 * @param  None
 ...
 *         and temperature that influence the frequency of the internal HSI
 * RC.
 ...
 * modes. It is used (enabled by hardware) as system clock source after
 * startup from Reset, wakeup from STOP and STANDBY mode, or in case of
 * failure of the HSE used directly or indirectly as system clock (if the
 * Clock Security System CSS is enabled).
 * @note   HSI can not be stopped if it is used as system clock source. In
 * this case, you have to select another source of the system clock then stop
 * the HSI.
 * @note   After enabling the HSI, the application software should wait on
 * HSIRDY flag to be set indicating that HSI clock is stable and can be used
 * as system clock source.
 * @param  NewState: new state of the HSI.
 *          This parameter can be: ENABLE or DISABLE.
 * @note   When the HSI is stopped, HSIRDY flag goes low after 6 HSI
 ...
 * application software should wait on LSERDY flag to be set indicating that
 * LSE clock is stable and can be used to clock the RTC.
 ...
 *          This parameter can be: ENABLE or DISABLE.
 * @note   When the LSI is stopped, LSIRDY flag goes low after 6 LSI
 * oscillator clock cycles.
 * @retval None
 */
 ...
 *
 * @param  PLLR: specifies the division factor for I2S, SAI, SYSTEM, SPDIF in
 * STM32F446xx devices This parameter must be a number between 2 and 7.
 *
...
#endif /* STM32F40_41xxx || STM32F427_437xx || STM32F429_439xx ||            \
          STM32F401xx || STM32F411xE */

/**
 * @brief  Enables or disables the main PLL.
 * @note   After enabling the main PLL, the application software should wait
 * on PLLRDY flag to be set indicating that PLL clock is stable and can be
 ...
 *
 * @note   This function can be used only for STM32F405xx/407xx,
 * STM32F415xx/417xx or STM32F401xx devices.
 *
 ...
 * clock This parameter must be a number between 50 and 432.
 ...
 * MHz on the I2S clock frequency.
 *
 * @retval None
 */
 ...
 * @note   You have to set the PLLI2SR parameter correctly to not exceed 192
 * MHz on the I2S clock frequency.
 * @note   the PLLI2SR parameter is only available with STM32F42xxx/43xxx
 * devices.
 *
 * @retval None
 */
void RCC_PLLI2SConfig(uint32_t PLLI2SM, uint32_t PLLI2SN, uint32_t PLLI2SP,
                      uint32_t PLLI2SQ, uint32_t PLLI2SR) {
	/* Check the parameters */
	assert_param(IS_RCC_PLLI2SM_VALUE(PLLI2SM));
	assert_param(IS_RCC_PLLI2SN_VALUE(PLLI2SN));
 ...
 *         RCC_PLLConfig function )
 *
 * @param  PLLSAIN: specifies the multiplication factor for PLLSAI VCO output
 * clock This parameter must be a number between 50 and 432.
 ...
 * @retval None
 */
void RCC_PLLSAIConfig(uint32_t PLLSAIN, uint32_t PLLSAIP, uint32_t PLLSAIQ,
                      uint32_t PLLSAIR) {
	/* Check the parameters */
	...
	RCC->PLLSAICFGR = (PLLSAIN << 6) | (((PLLSAIP >> 1) - 1) << 16)
	                | (PLLSAIQ << 24) | (PLLSAIR << 28);
}
 ...
 * VCO input frequency ranges from 1 to 2 MHz. It is recommended to select a
 * frequency of 2 MHz to limit PLLSAI jitter.
 *
 * @param  PLLSAIN: specifies the multiplication factor for PLLSAI VCO output
 * clock This parameter must be a number between 50 and 432.
 * @note   You have to set the PLLSAIN parameter correctly to ensure that the
 ...
 * @retval None
 */
void RCC_PLLSAIConfig(uint32_t PLLSAIM, uint32_t PLLSAIN, uint32_t PLLSAIP,
                      uint32_t PLLSAIQ) {
	/* Check the parameters */
	...
	RCC->PLLSAICFGR = PLLSAIM | (PLLSAIN << 6) | (((PLLSAIP >> 1) - 1) << 16)
	                | (PLLSAIQ << 24);
}
#endif /* STM32F446xx */
 ...
 * modes.
 * @param  NewState: new state of the PLLSAI. This parameter can be: ENABLE or
 * DISABLE.
 * @retval None
 */
 ...
 * oscillator is automatically disabled and an interrupt is generated to
 * inform the software about the failure (Clock Security System Interrupt,
 * CSSI), allowing the MCU to perform rescue operations. The CSSI is linked to
 ...
 *            @arg RCC_MCO2SOURCE_I2SCLK: I2SCLK clock selected as MCO2
 * source, available only for STM32F410xx devices
 ...
 * @note  For STM32F410xx devices to output I2SCLK clock on MCO2 you should
 * have at last one of the SPI clocks enabled (SPI1, SPI2 or SPI5).
 * @retval None
 */
...
through configurable prescaler and used to clock the CPU, memory and
...
RCC_I2SCLKConfig() function to configure this clock.
        (+@) RTC: the RTC clock can be derived either from the LSI, LSE or HSE
clock divided by 2 to 31. You have to use RCC_RTCCLKConfig() and
RCC_RTCCLKCmd() functions to configure this clock.
        (+@) USB OTG FS, SDIO and RTC: USB OTG FS require a frequency equal to
48 MHz to work correctly, while the SDIO require a frequency equal or lower
than to 48. This clock is derived of the main PLL through PLLQ divider.
        (+@) IWDG clock which is always the LSI clock.

      (#) For STM32F405xx/407xx and STM32F415xx/417xx devices, the maximum
...
|---------------------------------------------------------------------| | |
voltage range  | voltage range  | voltage range   | voltage range   | | | 2.7
V - 3.6 V  | 2.4 V - 2.7 V  | 2.1 V - 2.4 V   | 1.8 V - 2.1 V   |
 |---------------|----------------|----------------|-----------------|-----------------|
 |0WS(1CPU cycle)|0 < HCLK <= 30  |0 < HCLK <= 24  |0 < HCLK <= 22   |0 < HCLK
<= 20   |
 |---------------|----------------|----------------|-----------------|-----------------|
 |1WS(2CPU cycle)|30 < HCLK <= 60 |24 < HCLK <= 48 |22 < HCLK <= 44  |20 <
HCLK <= 40  |
 |---------------|----------------|----------------|-----------------|-----------------|
 |2WS(3CPU cycle)|60 < HCLK <= 90 |48 < HCLK <= 72 |44 < HCLK <= 66  |40 <
HCLK <= 60  |
 |---------------|----------------|----------------|-----------------|-----------------|
 |3WS(4CPU cycle)|90 < HCLK <= 120|72 < HCLK <= 96 |66 < HCLK <= 88  |60 <
HCLK <= 80  |
 |---------------|----------------|----------------|-----------------|-----------------|
 |4WS(5CPU cycle)|120< HCLK <= 150|96 < HCLK <= 120|88 < HCLK <= 110 |80 <
HCLK <= 100 |
 |---------------|----------------|----------------|-----------------|-----------------|
 |5WS(6CPU cycle)|150< HCLK <= 168|120< HCLK <= 144|110 < HCLK <= 132|100 <
HCLK <= 120|
 |---------------|----------------|----------------|-----------------|-----------------|
 |6WS(7CPU cycle)|      NA        |144< HCLK <= 168|132 < HCLK <= 154|120 <
HCLK <= 140|
 |---------------|----------------|----------------|-----------------|-----------------|
 |7WS(8CPU cycle)|      NA        |      NA        |154 < HCLK <= 168|140 <
HCLK <= 160|
 +---------------|----------------|----------------|-----------------|-----------------+
      (#) For STM32F42xxx/43xxx/469xx/479xx devices, the maximum frequency of
the SYSCLK and HCLK is 180 MHz, PCLK2 90 MHz and PCLK1 45 MHz. Depending on
the device voltage range, the maximum frequency should be adapted accordingly:
 +-------------------------------------------------------------------------------------+
 | Latency       |                HCLK clock frequency (MHz) | |
|---------------------------------------------------------------------| | |
voltage range  | voltage range  | voltage range   | voltage range   | | | 2.7
...
HCLK <= 100 |
 |---------------|----------------|----------------|-----------------|-----------------|
 |5WS(6CPU cycle)|120< HCLK <= 180|120< HCLK <= 144|110 < HCLK <= 132|100 <
HCLK <= 120|
 |---------------|----------------|----------------|-----------------|-----------------|
 |6WS(7CPU cycle)|      NA        |144< HCLK <= 168|132 < HCLK <= 154|120 <
HCLK <= 140|
 |---------------|----------------|----------------|-----------------|-----------------|
 |7WS(8CPU cycle)|      NA        |168< HCLK <= 180|154 < HCLK <= 176|140 <
HCLK <= 160|
 |---------------|----------------|----------------|-----------------|-----------------|
 |8WS(9CPU cycle)|      NA        |      NA        |176 < HCLK <= 180|160 <
HCLK <= 168|
 +-------------------------------------------------------------------------------------+

      (#) For STM32F401xx devices, the maximum frequency of the SYSCLK and
HCLK is 84 MHz, PCLK2 84 MHz and PCLK1 42 MHz. Depending on the device voltage
range, the maximum frequency should be adapted accordingly:
 +-------------------------------------------------------------------------------------+
 | Latency       |                HCLK clock frequency (MHz) | |
|---------------------------------------------------------------------| | |
voltage range  | voltage range  | voltage range   | voltage range   | | | 2.7
...
HCLK <= 40  |
 |---------------|----------------|----------------|-----------------|-----------------|
 |2WS(3CPU cycle)|60 < HCLK <= 84 |48 < HCLK <= 72 |44 < HCLK <= 66  |40 <
HCLK <= 60  |
 |---------------|----------------|----------------|-----------------|-----------------|
 |3WS(4CPU cycle)|      NA        |72 < HCLK <= 84 |66 < HCLK <= 84  |60 <
HCLK <= 80  |
 |---------------|----------------|----------------|-----------------|-----------------|
 |4WS(5CPU cycle)|      NA        |      NA        |      NA         |80 <
HCLK <= 84  |
 +-------------------------------------------------------------------------------------+

      (#) For STM32F410xx/STM32F411xE devices, the maximum frequency of the
SYSCLK and HCLK is 100 MHz, PCLK2 100 MHz and PCLK1 50 MHz. Depending on the
device voltage range, the maximum frequency should be adapted accordingly:
 +-------------------------------------------------------------------------------------+
 | Latency       |                HCLK clock frequency (MHz) | |
|---------------------------------------------------------------------| | |
voltage range  | voltage range  | voltage range   | voltage range   | | | 2.7
...
<= 16   |
 |---------------|----------------|----------------|-----------------|-----------------|
 |1WS(2CPU cycle)|30 < HCLK <= 64 |24 < HCLK <= 48 |18 < HCLK <= 36  |16 <
HCLK <= 32  |
 |---------------|----------------|----------------|-----------------|-----------------|
 |2WS(3CPU cycle)|64 < HCLK <= 90 |48 < HCLK <= 72 |36 < HCLK <= 54  |32 <
HCLK <= 48  |
 |---------------|----------------|----------------|-----------------|-----------------|
 |3WS(4CPU cycle)|90 < HCLK <= 100|72 < HCLK <= 96 |54 < HCLK <= 72  |48 <
HCLK <= 64  |
 |---------------|----------------|----------------|-----------------|-----------------|
 |4WS(5CPU cycle)|      NA        |96 < HCLK <= 100|72 < HCLK <= 90  |64 <
HCLK <= 80  |
 |---------------|----------------|----------------|-----------------|-----------------|
 |5WS(6CPU cycle)|      NA        |       NA       |90 < HCLK <= 100 |80 <
HCLK <= 96  |
 |---------------|----------------|----------------|-----------------|-----------------|
 |6WS(7CPU cycle)|      NA        |       NA       |        NA       |96 <
HCLK <= 100 |
 +-------------------------------------------------------------------------------------+
 ...
 * RCC_GetSYSCLKSource() function to know which clock is currently used as
 * system clock source.
 ...
 * source (RCC_SYSCLKSource_PLLPCLK for STM32F446xx devices)
 *            @arg RCC_SYSCLKSource_PLLRCLK: PLL R selected as system clock
 * source only for STM32F412xG, STM32F413_423xx and STM32F446xx devices
 * @retval None
 */
 ...
 *              - 0x08: PLL used as system clock (PLL P for STM32F446xx
 * devices)
 *              - 0x0C: PLL R used as system clock (only for STM32F412xG,
 * STM32F413_423xx and STM32F446xx devices)
 */
 ...
 * correctly these bits to ensure that HCLK not exceed the maximum allowed
 * frequency (for more details refer to section above "CPU, AHB and APB busses
 * clocks configuration functions")
 * @param  RCC_SYSCLK: defines the AHB clock divider. This clock is derived
 * from the system clock (SYSCLK). This parameter can be one of the following
 * values:
 ...
 * from the AHB clock (HCLK). This parameter can be one of the following
 * values:
 ...
 * function must be called to update the structure's field. Otherwise, any
 *         configuration based on this function will be incorrect.
 *
 * @retval None
 */
void RCC_GetClocksFreq(RCC_ClocksTypeDef *RCC_Clocks) {
	uint32_t tmp = 0, presc = 0, pllvco = 0, pllp = 2, pllsource = 0,
	         pllm = 2;
#if defined(STM32F412xG) || defined(STM32F413_423xx) || defined(STM32F446xx)
	uint32_t pllr = 2;
#endif /* STM32F412xG || STM32F413_423xx || STM32F446xx */
...
clocks.
      ...
      (#) The RTC clock which is derived from the LSI, LSE or HSE clock
divided by 2 to 31.

      (#) After restart from Reset or wakeup from STANDBY, all peripherals are
off except internal SRAM, Flash and JTAG. Before to start using a peripheral
...
clocks can be disabled prior to executing the WFI or WFE instructions. You can
do this using RCC_AHBPeriphClockLPModeCmd(), RCC_APB2PeriphClockLPModeCmd()
and RCC_APB1PeriphClockLPModeCmd() functions.
 ...
 * @note   The maximum input clock frequency for RTC is 1MHz (when using HSE
 * as RTC clock source).
 *
 * @retval None
 */
void RCC_RTCCLKConfig(uint32_t RCC_RTCCLKSource) {
	...
	if ((RCC_RTCCLKSource & 0x00000300)
	    == 0x00000300) { /* If HSE is selected as RTC clock source, configure
 ...
 *            @arg RCC_I2SCLKSource_HSI_HSE: HSI or HSE depends on PLLSRC used
 * as I2S clock source
 * @retval None
 */
 ...
 *          This parameter can be one of the following values:
 *            @arg RCC_SAICLKSource_PLLSAI: PLLSAI clock used as SAI clock
 * source
 *            @arg RCC_SAICLKSource_PLLI2S: PLLI2S clock used as SAI clock
 * source
 *            @arg RCC_SAICLKSource_PLL: PLL clock used as SAI clock source
 *            @arg RCC_SAICLKSource_HSI_HSE: HSI or HSE depends on PLLSRC used
 * as SAI clock source
 * @retval None
 */
 ...
 *          This parameter can be one of the following values:
 *            @arg RCC_SAIACLKSource_PLLI2SR: PLLI2SR clock used as SAI clock
 * source
 *            @arg RCC_SAIACLKSource_PLLI2S: PLLI2S clock used as SAI clock
 * source
 *            @arg RCC_SAIACLKSource_PLL: PLL clock used as SAI clock source
 *            @arg RCC_SAIACLKSource_HSI_HSE: HSI or HSE depends on PLLSRC
 * used as SAI clock source
 * @retval None
 */
 ...
 *          This parameter can be one of the following values:
 *            @arg RCC_SAIBCLKSource_PLLI2SR: PLLI2SR clock used as SAI clock
 * source
 *            @arg RCC_SAIBCLKSource_PLLI2S: PLLI2S clock used as SAI clock
 * source
 *            @arg RCC_SAIBCLKSource_PLL: PLL clock used as SAI clock source
 *            @arg RCC_SAIBCLKSource_HSI_HSE: HSI or HSE depends on PLLSRC
 * used as SAI clock source
 * @retval None
 */
 ...
 * source
 *            @arg RCC_I2S2CLKSource_Ext: External clock mapped on the
 ...
 *            @arg RCC_SAIACLKSource_Ext: External clock mapped on the
 * I2S_CKIN pin used as SAI1 Block A clock
 * @retval None
 */
void RCC_SAIBlockACLKConfig(uint32_t RCC_SAIBlockACLKSource) {
	uint32_t tmpreg = 0;
 ...
 *            @arg RCC_SAIBCLKSource_Ext: External clock mapped on the
 * I2S_CKIN pin used as SAI1 Block B clock
 * @retval None
 */
void RCC_SAIBlockBCLKConfig(uint32_t RCC_SAIBlockBCLKSource) {
	uint32_t tmpreg = 0;
 ...
 * . This parameter must be a number between 1 and 32. SAI1 clock frequency =
 * f(PLLI2S_Q) / RCC_PLLI2SDivQ
 *
 * @retval None
 */
 ...
 *
 * @param   RCC_PLLI2SDivR: specifies the PLLI2S division factor for SAI1
 * clock. This parameter must be a number between 1 and 32. SAI1 clock
 * frequency = f(PLLI2SR) / RCC_PLLI2SDivR
 * @retval None
 */
 ...
 * . LTDC clock frequency = f(PLLSAI_R) / RCC_PLLSAIDivR This parameter can be
 ...
 *          This parameter can be one of the following values:
 *            @arg RCC_DFSDMCLKSource_APB: APB clock used as DFSDM clock
 * source.
 *            @arg RCC_DFSDMCLKSource_SYS: System clock used as DFSDM clock
 * source.
 *
 * @retval None
 */
 ...
 *          This parameter can be one of the following values:
 *            @arg RCC_DFSDM1AUDIOCLKSOURCE_I2SAPB1: APB clock used as DFSDM
 * clock source.
 *            @arg RCC_DFSDM1AUDIOCLKSOURCE_I2SAPB2: System clock used as
 * DFSDM clock source.
 *
 * @retval None
 */
 ...
 *          This parameter can be one of the following values:
 *            @arg RCC_DFSDM2AUDIOCLKSOURCE_I2SAPB1: APB clock used as DFSDM
 * clock source.
 *            @arg RCC_DFSDM2AUDIOCLKSOURCE_I2SAPB2: System clock used as
 * DFSDM clock source.
 *
 * @retval None
 */
 ...
 * selection This parameter can be one of the following values:
 *            @arg RCC_TIMPrescDesactivated: The Timers kernels clocks
 * prescaler is equal to HPRE if PPREx is corresponding to division by 1 or 2,
 ...
 * is equal to HPRE if PPREx is corresponding to division by 1, 2 or 4, else
 * it is equal to [(HPRE * PPREx) / 4] if PPREx is corresponding to division
 * by 8 or more.
 * @retval None
 */
 ...
 *            @arg RCC_AHB1Periph_GPIOJ:       GPIOJ clock (STM32F42xxx/43xxx
 * devices)
 *            @arg RCC_AHB1Periph_GPIOK:       GPIOK clock (STM32F42xxx/43xxx
 * devices)
 ...
 *            @arg RCC_AHB1Periph_DMA2D:       DMA2D clock (STM32F429xx/439xx
 * devices)
...
void RCC_AHB2PeriphClockCmd(uint32_t RCC_AHB2Periph,
    ...
    || defined(STM32F413_423xx) || defined(STM32F427_437xx)                  \
    || defined(STM32F429_439xx) || defined(STM32F446xx)                      \
    || defined(STM32F469_479xx)
/**
 ...
 *           - RCC_AHB3Periph_QSPI
 * (STM32F412xG/STM32F413_423xx/STM32F446xx/STM32F469_479xx devices)
 * @param  NewState: new state of the specified peripheral clock.
 *          This parameter can be: ENABLE or DISABLE.
 * @retval None
 */
void RCC_AHB3PeriphClockCmd(uint32_t RCC_AHB3Periph,
	...
	if (NewState != DISABLE) {
		...
		RCC->AHB3ENR &= ~RCC_AHB3Periph;
          ...
          STM32F427_437xx || STM32F429_439xx || STM32F446xx ||               \
          STM32F469_479xx */
 ...
 *            @arg RCC_APB1Periph_LPTIM1: LPTIM1 clock (STM32F410xx and
 ...
 * (STM32F42xxx/43xxx/446xx/469xx/479xx/413_423xx devices)
 *            @arg RCC_APB2Periph_SAI2:   SAI2 clock (STM32F446xx devices)
 *            @arg RCC_APB2Periph_LTDC:   LTDC clock (STM32F429xx/439xx
 * devices)
 *            @arg RCC_APB2Periph_DSI:    DSI clock (STM32F469_479xx devices)
 *            @arg RCC_APB2Periph_DFSDM1: DFSDM Clock (STM32F412xG and
 * STM32F413_423xx Devices)
 *            @arg RCC_APB2Periph_DFSDM2: DFSDM2 Clock (STM32F413_423xx
 * Devices)
 *            @arg RCC_APB2Periph_UART9:  UART9 Clock (STM32F413_423xx
 * Devices)
 *            @arg RCC_APB2Periph_UART10: UART10 Clock (STM32F413_423xx
 * Devices)
 * @param  NewState: new state of the specified peripheral clock.
 *          This parameter can be: ENABLE or DISABLE.
 * @retval None
 ...
 *          This parameter must be:
 *           - RCC_AHB3Periph_FSMC or RCC_AHB3Periph_FMC (STM32F412xG,
 * STM32F413_423xx and STM32F429x/439x devices)
 *           - RCC_AHB3Periph_QSPI (STM32F412xG/STM32F446xx/STM32F469_479xx
 * devices)
 * @param  NewState: new state of the specified peripheral reset.
 *          This parameter can be: ENABLE or DISABLE.
 * @retval None
 ...
 *         power consumption.
 * @note   After wakeup from SLEEP mode, the peripheral clock is enabled
 * again.
 ...
 *            @arg RCC_APB1Periph_SPI2:   SPI2 clock
 *            @arg RCC_APB1Periph_SPI3:   SPI3 clock
 *            @arg RCC_APB1Periph_SPDIF:   SPDIF RX clock (STM32F446xx
 * devices)
 *            @arg RCC_APB1Periph_USART2: USART2 clock
 *            @arg RCC_APB1Periph_USART3: USART3 clock
 *            @arg RCC_APB1Periph_UART4:  UART4 clock
 ...
 * STM32F410xx/STM32F411xx/STM32F446xx/STM32F469_479xx devices.
 ...
 *          This parameter can be one of the following values:
 *            @arg RCC_DSICLKSource_PHY: DSI-PHY used as DSI byte lane clock
 * source (usual case).
 *            @arg RCC_DSICLKSource_PLLR: PLL_R used as DSI byte lane clock
 * source, used in case DSI PLL and DSI-PHY are off (low power mode).
 * @retval None
 */
 ...
 * @note This feature is only available for STM32F446xx/STM32F469_479xx
 * devices.
 ...
 * @note This feature is only available for STM32F469_479xx/STM32F446xx
 * devices.
 ...
 *          This parameter can be one of the following values:
 *            @arg RCC_SPDIFRXCLKSource_PLLR: SPDIFRX clock from PLL_R
 * selected.
 *            @arg RCC_SPDIFRXCLKSource_PLLI2SP: SPDIFRX clock from PLLI2S_P
 * selected.
 * @retval None
 */
 ...
 *          This parameter can be one of the following values:
 *            @arg RCC_CECCLKSource_HSIDiv488: CEC clock from HSI/488
 * selected.
 ...
 *          This parameter can be one of the following values:
 *            @arg RCC_FMPI2C1CLKSource_APB1: FMPI2C1 clock from APB1
 * selected.
 *            @arg RCC_FMPI2C1CLKSource_SYSCLK: FMPI2C1 clock from Sytem clock
 * selected.
