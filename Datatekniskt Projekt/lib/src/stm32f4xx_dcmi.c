/**
 ...
 ===============================================================================
    [..]
      ...
      (#) Enable the clock for the DCMI and associated GPIOs using the
 following functions: RCC_AHB2PeriphClockCmd(RCC_AHB2Periph_DCMI, ENABLE);
          RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOx, ENABLE);

      (#) DCMI pins configuration
        (++) Connect the involved DCMI pins to AF13 using the following
 function GPIO_PinAFConfig(GPIOx, GPIO_PinSourcex, GPIO_AF_DCMI);
      ...
      -@-  If you need to capture only a rectangular window from the received
           ...
           In this case, the Crop configuration should be made before to
 enable and start the DCMI interface.
 ...
 * @param  DCMI_InitStruct : pointer to a DCMI_InitTypeDef structure which
 * will be initialized.
 * @retval None
 */
 ...
 * that contains the configuration information for the DCMI peripheral CROP
 * mode.
 * @retval None
 */
void DCMI_CROPConfig(DCMI_CROPInitTypeDef *DCMI_CROPInitStruct) {
	...
	DCMI->CWSTRTR =
	    (uint32_t) ((uint32_t) DCMI_CROPInitStruct->DCMI_HorizontalOffsetCount
	                | ((uint32_t) DCMI_CROPInitStruct->DCMI_VerticalStartLine
	                   << 16));

	/* Sets the CROP window size */
	DCMI->CWSIZER =
	    (uint32_t) (DCMI_CROPInitStruct->DCMI_CaptureCount
	                | ((uint32_t) DCMI_CROPInitStruct->DCMI_VerticalLineCount
	                   << 16));
}
 ...
 * that contains the embedded synchronization codes for the DCMI peripheral.
 * @retval None
 */
void DCMI_SetEmbeddedSynchroCodes(
	...
	DCMI->ESCR =
	    (uint32_t) (DCMI_CodesInitStruct->DCMI_FrameStartCode
	                | ((uint32_t) DCMI_CodesInitStruct->DCMI_LineStartCode
	                   << 8)
	                | ((uint32_t) DCMI_CodesInitStruct->DCMI_LineEndCode
	                   << 16)
	                | ((uint32_t) DCMI_CodesInitStruct->DCMI_FrameEndCode
	                   << 24));
}
