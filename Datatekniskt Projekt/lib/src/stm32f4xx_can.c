...
maximum number of time quanta to perform resynchronization, the number of time
...
void CAN_FilterInit(CAN_TypeDef *CANx,
	...
	/* Check the parameters */
	assert_param(
	    IS_CAN_FILTER_NUMBER(CAN_FilterInitStruct->CAN_FilterNumber));
 ...
 *          This parameter can be: ENABLE (CAN reception/transmission is
 * frozen during debug. Reception FIFOs can still be accessed/controlled
 ...
 * @param  CANx: where x can be 1,2 or 3 to select the CAN peripheral.
 * @param  TxMessage: pointer to a structure which contains CAN Id, CAN DLC
 * and CAN data.
 * @note   CAN3 peripheral is available only for STM32F413_423xx devices
...
uint8_t CAN_OperatingModeRequest(CAN_TypeDef *CANx,
	...
	if (CAN_OperatingMode == CAN_OperatingMode_Initialization) {
		/* Request initialisation */
		CANx->MCR =
		    (uint32_t) ((CANx->MCR & (uint32_t) (~(uint32_t) CAN_MCR_SLEEP))
		                | CAN_MCR_INRQ);
		...
		/* Request Sleep mode */
		CANx->MCR =
		    (uint32_t) ((CANx->MCR & (uint32_t) (~(uint32_t) CAN_MCR_INRQ))
		                | CAN_MCR_SLEEP);
...
receive FIFO. These Flags are cleared only by hardware.
...
passed the filter while the FIFO was full.

      (+) Operating Mode Flags

        (++) CAN_FLAG_WKU    : Wake up Flag
                               set to signal that a SOF bit has been detected
while the CAN hardware was in Sleep mode.
...
(Receive Error Counter or Transmit Error Counter greater than 96). This Flag
is cleared only by hardware.

        (++) CAN_FLAG_EPV    : Error Passive Flag
                               Set when the Error Passive limit has been
...
bus-off state is entered on TEC overflow, greater than 255. This Flag is
cleared only by hardware.

        (++) CAN_FLAG_LEC    : Last error code Flag
                               set If a message has been transferred
...
FIFO.
      ...
      (+) Operating Mode Interrupts
        ...
        (++) CAN_IT_WKU     :  Wake-up Interrupt
                               if enabled, this interrupt source is pending
when a SOF bit has been detected while the CAN hardware was in Sleep mode.

        (++) CAN_IT_SLK     :  Sleep acknowledge Interrupt
                               if enabled, this interrupt source is pending
when the CAN has entered Sleep Mode.

      (+) Error Interrupts

        (++) CAN_IT_EWG     :  Error warning Interrupt
                               if enabled, this interrupt source is pending
when the warning limit has been reached (Receive Error Counter or Transmit
Error Counter=96).
...
Transmit Error Counter>127).
        ...
        (++) CAN_IT_BOF     :  Bus-off Interrupt
                               if enabled, this interrupt source is pending
when CAN enters the bus-off state. The bus-off state is entered on TEC
overflow, greater than 255. This Flag is cleared only by hardware.

        (++) CAN_IT_LEC     :  Last error code Interrupt
                               if enabled, this interrupt source is pending
when a message has been transferred (reception or transmission) with error,
and the error code is hold.

        (++) CAN_IT_ERR     :  Error Interrupt
                               if enabled, this interrupt source is pending
when an error condition is pending.

    [..] Managing the CAN controller events :

         The user should identify which mode will be used in his application
to manage the CAN controller events: Polling mode or Interrupt mode.
...
ITStatus CAN_GetITStatus(CAN_TypeDef *CANx, uint32_t CAN_IT) {
	...
	if ((CANx->IER & CAN_IT) != RESET) {
		...
		case CAN_IT_TME:
			...
			itstatus = CheckITStatus(CANx->TSR, CAN_TSR_RQCP0 | CAN_TSR_RQCP1
			                                        | CAN_TSR_RQCP2);
			break;
		case CAN_IT_FMP0:
...
void CAN_ClearITPendingBit(CAN_TypeDef *CANx, uint32_t CAN_IT) {
	...
	case CAN_IT_EWG:
		...
		/* @note the corresponding Flag is cleared by hardware depending on
		 * the CAN Bus status*/
		break;
	case CAN_IT_EPV:
		/* Clear CAN_MSR_ERRI (rc_w1) */
