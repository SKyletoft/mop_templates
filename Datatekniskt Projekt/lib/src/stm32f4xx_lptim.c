...
================================================================================
           ...
           - Configure the clock source, the prescaler, the waveform shape and
             the output polarity by filling the "LPTIM_InitTypeDef" structure
and calling LPTIM_Init.
           - If the ULPTIM source is selected as clock source, configure the
digital Glitch filter by setting the number of consecutive samples to be
detected by using LPTIM_ConfigClockGlitchFilter.
           - To select a software start use LPTIM_SelectSoftwareStart.
           - To select an external trigger for the start of the counter,
configure the source and its active edge polarity by calling
             LPTIM_ConfigExternalTrigger. Configure the Digital Glitch filter
for the external triggers by setting the number of consecutive samples to be
detected by using LPTIM_ConfigTriggerGlitchFilter.
...
started from a trigger event and never stops until the timer is disabled
           ...
           - Configure the preload mode by using LPTIM_ConfigUpdate function.
2 modes are available:
               + The Autoreload and compare registers are updated immediately
after APB write.
               + The Autoreload and compare registers are updated at the end
of counter period.
            - Enable the peripheral by calling LPTIM_Cmd.

           Encoder mode:
           -------------
           - To select the encoder feature, use the function:
LPTIM_SelectEncoderMode.
           - To select on which edge (Rising edge, falling edge or both edges)
             the counter is incremented, use LPTIM_SelectClockPolarity.

           Counter mode:
           -------------
           - Use LPTIM_SelectCounterMode to select the counting mode. In this
mode the counter is incremented on each valid event on ULPTIM.

           Timeout function:
           -----------------
           In this case, the trigger will reset the timer. The first trigger
event will start the timer, any successive trigger event will reset the
counter and the timer restarts.
...
@verbatim
 ...
 ===============================================================================
  This section provides functions allowing to:
   ...
   - Initialize the Clock source, the Prescaler, the Ouput Waveform shape and
Polarity
...
/**
 * @brief  Deinitializes the LPTIMx peripheral registers to their default
 * reset values.
...
/**
 * @brief  Initializes the LPTIMx peripheral according to the specified
...
void LPTIM_Init(LPTIM_TypeDef *LPTIMx, LPTIM_InitTypeDef *LPTIM_InitStruct) {
	...
	assert_param(
	    IS_LPTIM_OUTPUT_POLARITY(LPTIM_InitStruct->LPTIM_OutputPolarity));
	...
	tmpreg1 |= (LPTIM_InitStruct->LPTIM_ClockSource
	            | LPTIM_InitStruct->LPTIM_Prescaler
	            | LPTIM_InitStruct->LPTIM_Waveform
	            | LPTIM_InitStruct->LPTIM_OutputPolarity);
 ...
 * @param  LPTIM_InitStruct : pointer to a LPTIM_InitTypeDef structure which
 * will be initialized.
 * @retval None
 */
...
@verbatim
 ...
 ===============================================================================
    ...
    - Configure the Glitch filter for the external clock and the external
clock.
 ...
 *         @arg LPTIM_ClockSource_APBClock_LPosc : APB clock/LP oscillators
 * selected
 ...
 * @note   It is mandatory to disable the peripheral to use this function.
 */
void LPTIM_SelectClockSource(LPTIM_TypeDef *LPTIMx,
 ...
 * @note   It is mandatory to disable the peripheral to use this function.
 */
void LPTIM_SelectULPTIMClockPolarity(LPTIM_TypeDef *LPTIMx,
 ...
 * @note   It is mandatory to disable the peripheral to use this function.
 */
void LPTIM_ConfigExternalTrigger(LPTIM_TypeDef *LPTIMx,
...
/**
 * @brief  Configures the digital filter for trigger by determining the number
 * of consecutive samples at the specified level to detect a correct
 * transition.
 * @param  LPTIMx: where x can be 1.
 * @param  LPTIM_TrigSampleTime: the number of samples to detect a valid
 * transition. This parameter can be:
 *     @arg LPTIM_TrigSampleTime_DirectTransistion : Event is detected on
 * input transitions
 *     @arg LPTIM_TrigSampleTime_2Transistions : Event is detected after 2
 * consecutive samples at the active level
 *     @arg LPTIM_TrigSampleTime_4Transistions : Event is detected after 4
 * consecutive samples at the active level
 *     @arg LPTIM_TrigSampleTime_8Transistions : Event is detected after 8
 * consecutive samples at the active level
 * @retval None
 *
 * @note   It is mandatory to disable the peripheral to use this function.
 ...
 */
void LPTIM_ConfigTriggerGlitchFilter(LPTIM_TypeDef *LPTIMx,
...
/**
  * @brief  Configures the digital filter for  the external clock by
  determining the number of consecutive samples at the specified level to
  detect a correct transition.
  * @param  LPTIMx: where x can be 1.
  * @param  LPTIM_ClockSampleTime: the number of samples to detect a valid
  transition.
  * This parameter can be:
  *     @arg LPTIM_ClockSampleTime_DirectTransistion : Event is detected on
  input transitions
  *     @arg LPTIM_ClockSampleTime_2Transistions : Event is detected after 2
  consecutive samples at the active level
  *     @arg LPTIM_ClockSampleTime_4Transistions : Event is detected after 4
  consecutive samples at the active level
  *     @arg LPTIM_ClockSampleTime_8Transistions : Event is detected after 8
  consecutive samples at the active level
  * @retval None
  *
  * @note   It is mandatory to disable the peripheral to use this function.
  * @note   An auxiliary clock must be present to use this feature.
  */
void LPTIM_ConfigClockGlitchFilter(LPTIM_TypeDef *LPTIMx,
 ...
 * @retval None
 */
void LPTIM_SetAutoreloadValue(LPTIM_TypeDef *LPTIMx,
...
/**
 * @brief  Enables or disables the Counter mode. When the Counter mode is
 * enabled, the counter is incremented each valid event on ULPTIM
 * @param  LPTIMx: where x can be 1.
 ...
 * @note   It is mandatory to disable the peripheral to use this function.
 */
void LPTIM_SelectCounterMode(LPTIM_TypeDef *LPTIMx,
 ...
 * @note   It is mandatory to disable the peripheral to use this function.
 */
void LPTIM_SelectEncoderMode(LPTIM_TypeDef *LPTIMx,
                             FunctionalState NewState) {
	/* Check the parameters */
	assert_param(IS_LPTIM_ALL_PERIPH(LPTIMx));
	assert_param(IS_FUNCTIONAL_STATE(NewState));
 ...
 *    @arg LPTIM_OP_TIM_DAC : Input coming from timer 6 output (for encoder
 * mode)
 * @retval Counter Register value
 */
...
@verbatim
 ===============================================================================
                  Interrupts and flags management functions
 ===============================================================================
  This section provides functions allowing to configure the LPTIM Interrupts,
get the status and clear flags bits.

  The LPTIM provides 7 Flags and Interrupts sources (2 flags and Interrupt
sources are available only on LPTIM peripherals equipped with encoder mode
interface)
  ...
  6. Direction change: from up to down [Available only for LPTIM peripheral
with encoder mode module]
  7. Direction change: from down to up [Available only for LPTIM peripheral
with encoder mode module]

  - To enable a specific interrupt source, use "LPTIM_ITConfig" function.
  - To check if an interrupt was occurred, call "LPTIM_GetITStatus" function
and read the returned value.
  - To get a flag status, call the "LPTIM_GetFlagStatus" function and read the
returned value.
 ...
 * @param  LPTIMx: where x can be 1.
 * @param  LPTIM_IT: specifies the TIM interrupts sources to be enabled or
 * disabled. This parameter can be any combination of the following values:
 *            @arg LPTIM_IT_DOWN: Counter direction change up to down
 * Interrupt source
 ...
 *            @arg LPTIM_CLEARF_UP: Counter direction change down to up Clear
 * Flag
 *            @arg LPTIM_CLEARF_ARROK: Autoreload register update OK Clear
 * Flag
 *            @arg LPTIM_CLEARF_CMPOK: Compare register update OK Clear Flag
 *            @arg LPTIM_CLEARF_EXTTRIG: External trigger edge event Clear
 * Flag
