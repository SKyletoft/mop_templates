...
typedef struct {
	...
	uint16_t
	    I2C_AcknowledgedAddress; /*!< Specifies if 7-bit or 10-bit address is
...
#define IS_I2C_REGISTER(REGISTER)                                            \
	(((REGISTER) == I2C_Register_CR1) || ((REGISTER) == I2C_Register_CR2)    \
	 ...
	 || ((REGISTER) == I2C_Register_OAR2) || ((REGISTER) == I2C_Register_DR) \
	 || ((REGISTER) == I2C_Register_SR1) || ((REGISTER) == I2C_Register_SR2) \
 ...
 *   peripheral (configured by the function I2C_OwnAddress2Config() and
 * enabled by the function I2C_DualAddressCmd()) the events
 * I2C_EVENT_SLAVE_XXX_SECONDADDRESS_MATCHED (where XXX could be TRANSMITTER
 * or RECEIVER) are set.
 *
 * 3) In case the address sent by the master is General Call (address 0x00)
 ...
 * similar. The second one can optionally be used when the user software
 * doesn't guarantee the EV3 is managed before the current byte end of
 * transfer.
...
/*
 ...
 event (can be the combination of one or more flags). It returns SUCCESS if
 the current status includes the given flags and returns ERROR if one or more
 flags are missing in the current status.
 ...
 monitored flags), the I2C_CheckEvent() function may return SUCCESS despite
 ...
 it allows to overcome the mentioned limitation of I2C_GetFlagStatus()
               function.
             - The returned value could be compared to events already defined
 in this file or to custom values defined by user. This function is suitable
 when multiple flags are monitored at the same time.
             - At the opposite of I2C_CheckEvent() function, this function
 allows user to choose when an event is accepted (when all events flags are
               set and no other flags are set or just when the needed flags
 are set like I2C_CheckEvent() function.

          - Limitations
             - User may need to define his own events.
             - Same remark concerning the error management is applicable for
 this function if user decides to check only regular communication flags (and
 ignores error flags).
