...
typedef struct {
	...
	uint32_t
	    QSPI_ComConfig_SIOOMode; /* Specifies the Send Instruction Only Once
	...
	uint32_t QSPI_ComConfig_DummyCycles; /* Specifies the Number of Dummy
	                                    Cycles. This parameter can be a number
...
typedef struct {
	...
	uint32_t QSPI_Prescaler; /* Specifies the prescaler value used to divide
	                            the QSPI clock.
	...
	uint32_t QSPI_FSize;   /* Specifies the Flash Size.
	                           QSPI_FSize+1 is effectively the number of address
	                          bits   required to address the flash memory.   The
	                          flash   capacity can be up to 4GB (addressed using
	                          32 bits) in   indirect mode, but the   addressable
	                          space in   memory-mapped mode is limited to 512MB
...
#define IS_QSPI_SSHIFT(SSHIFT)                                               \
	(((SSHIFT) == QSPI_SShift_NoShift)                                       \
	 || ((SSHIFT) == QSPI_SShift_HalfCycleShift))
/* Legacy Defines */
...
#define QSPI_CSHTime_4Cycle                                                  \
	((uint32_t) QUADSPI_DCR_CSHT_0 | QUADSPI_DCR_CSHT_1)
#define QSPI_CSHTime_5Cycle ((uint32_t) QUADSPI_DCR_CSHT_2)
#define QSPI_CSHTime_6Cycle                                                  \
	((uint32_t) QUADSPI_DCR_CSHT_2 | QUADSPI_DCR_CSHT_0)
#define QSPI_CSHTime_7Cycle                                                  \
	((uint32_t) QUADSPI_DCR_CSHT_2 | QUADSPI_DCR_CSHT_1)
...
#define IS_QSPI_FSEL(FLA)                                                    \
	(((FLA) == QSPI_FSelect_1) || ((FLA) == QSPI_FSelect_2))
/**
 * @}
 */
...
#define IS_QSPI_DFM(FLA)                                                     \
	(((FLA) == QSPI_DFlash_Enable) || ((FLA) == QSPI_DFlash_Disable))
/**
 * @}
 */
